...
Deliberately NOT a new required status check: this job is registered in both
:data:`scripts.ci.ci_summary_gate.GATE_JOBS` (the completeness anchor — blocks
a premature green before this job has completed) and
:data:`scripts.ci.ci_summary_gate.STRICT_SUCCESS_JOBS` (requires an exact
``success`` conclusion; a ``skipped``/``cancelled`` run — which should never
happen since the job carries no ``if:`` — fails closed rather than being
silently tolerated), and enforced through the existing fail-closed
``CI Summary`` umbrella poller. Adding a new top-level required context that
does not report on every PR shape wedges merges indefinitely (see CLAUDE.md,
deploy-gate section); the umbrella pattern has no such failure mode because its
check-run always instantiates.

Port note (OMN-15224): this module is ported verbatim from the canary landing
on omnibase_infra (OMN-15214, PR #2486) — the verdict logic, evaluate_once
contract, and CLI are unchanged. Only this docstring's registration paragraph
was adapted, because omniclaude's ``ci_summary_gate.py`` uses a flat
``GATE_JOBS``/``STRICT_SUCCESS_JOBS`` split rather than omnibase_infra's
``STRICT_GATE_JOBS``/``SKIPPABLE_GATE_JOBS`` split.
