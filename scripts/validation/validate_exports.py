...
Usage:
    uv run python scripts/validation/validate_exports.py
    uv run python scripts/validation/validate_exports.py --verbose
    uv run python scripts/validation/validate_exports.py --warn-missing
    uv run python scripts/validation/validate_exports.py --fail-on-star
    uv run python scripts/validation/validate_exports.py src/omniclaude/hooks/
...
def should_exclude_file(filepath: Path) -> bool:
    ...
    if "archived" in parts or "archive" in parts or "_archive" in parts:
        return True

    # Exclude lib/ directory (legacy code, will be deleted post-Beta)
    if "lib" in parts:
        return True
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=Path,
        help="Files or directories to check (default: src/omniclaude/)",
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Source directory (default: src/omniclaude/)",
    )
    parser.add_argument(
        "--fail-on-star",
        action="store_true",
    ...
    if args.paths:
        ...
        cwd = Path.cwd()
        src_dir = cwd / "src" / "omniclaude"

        if not src_dir.exists():
            # Try from script location
            script_dir = Path(__file__).parent
            src_dir = script_dir.parent.parent / "src" / "omniclaude"
