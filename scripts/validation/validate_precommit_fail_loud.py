...
"""
CI/pre-commit check: fail-loud meta-gate over .pre-commit-config.yaml (OMN-14655).

DRIFT-2 (false-green hook) recurrence guard. Two hooks in this repo
(`skill-contract-validation`, `no-cloud-bus`) silently degraded to `exit 0`
when a sibling script could not be resolved -- a skipped gate that was
byte-indistinguishable from a passing one. Both were fixed in OMN-14655.
This gate makes the fix a mechanism, not a one-off: it hard-fails (a) any
gate wrapper script that pairs "not found" / WARN / SKIP language with a
following `exit 0` (the recurring false-green shape), and (b) any hook
`stages:` value that `default_install_hook_types` does not cover, so a
pre-push/commit-msg hook can never silently go uninstalled again.
...
def _resolve_script(entry: str) -> Path | None:
    ...
    for token in entry.replace("\n", " ").split():
        if token.endswith((".sh", ".py")) and (
            token.startswith("scripts/") or token.startswith(".pre-commit-hooks/")
        ):
...
def _iter_local_hooks(config: dict) -> list[dict]:
    hooks: list[dict] = []
...
def check_fail_loud(config: dict) -> list[str]:
    violations: list[str] = []
...
def check_stage_coverage(config: dict) -> list[str]:
