...
EOSQL
    ...
    for migration in "$MIGRATIONS_DIR"/*.sql; do
        ...
        if [ -f "$migration" ]; then
            ...
            already_applied=$(psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" --host="$POSTGRES_HOST" -v migration_name="$migration_name" -tAc "SELECT 1 FROM schema_migrations WHERE filename = :'migration_name' LIMIT 1;")
            ...
            psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" --host="$POSTGRES_HOST" -v migration_name="$migration_name" -c "INSERT INTO schema_migrations (filename) VALUES (:'migration_name');"
        fi
    done
else
