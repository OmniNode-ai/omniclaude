...
| `plan-verified-state-gate.yml` | Plan Verified State Gate | `plan_to_tickets` plans under `docs/plans/` or `docs/tracking/` that lack a fresh `## Current Verified State` section with a `verified: <date> via <command>` line dated within the freshness window (OMN-13336). Grandfather allowlist: `.onex_ratchets/plan_verified_state_allowlist.yaml` (burn-down only). |
