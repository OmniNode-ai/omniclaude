...
- PreToolUse: `pre_tool_use_done_flip_guard.sh` (OMN-13856 Done-flip durable-evidence guard), `pre_tool_use_worktree_guard.sh` (OMN-14330 worktree canonical-root guard), and `pre_tool_use_lane_open.sh` (matcher `^(Task|Agent|Workflow)$`, OMN-16471 — writes a durable OPEN lane record at the dispatch seam. A pure **observer**: always exits 0 and always emits nothing, so it can neither refuse a dispatch nor inject context into the launching turn)
