...
**Lane-name attribution at close time (OMN-18690).** `SubagentStop` carries no `tool_input`, so the closer used to fall through to `agent_id` — the harness spells a named teammate's id as `a` + the lane name + `-` + 16 hex — which matches no OPEN record by name, sending every death to a synthetic `unattributed-*` id while the real dispatch record aged into `died_no_terminal`. One lane, two failure rows, neither carrying its tickets. The guard now reads the dispatch-time name off the harness's own `agent-<agent id>.meta.json` sidecar beside the transcript (`name`, then `agentType`, then `description` — the same precedence `extract_lane_name` uses at dispatch), and `lane_registry` additionally matches the name an agent id *spells* against names already recorded. Both are lookups against recorded truth: an id naming no open lane still falls through to `unattributed-*`, which is why neither can close the wrong record. Historical pairs are repaired by `plugins/onex/hooks/scripts/onex-lane-pair-reconcile.sh` — dry-run by default, and it **appends** a resolution to `$ONEX_STATE_DIR/hooks/lanes/resolutions.jsonl` that `reconcile` applies as a read-time overlay. It never edits a lane record: those are governed evidence, and a repair that rewrote them would be indistinguishable from the corruption it repairs. It also refuses to resolve an open record still inside its TTL, because a twin can be written at a usage-limit pause the lane later resumes from.
