...
"""Delegation runners — Bifrost gateway adapter and in-process pipeline.

This module exposes two runners:

* ``DelegationRunner`` — production-canonical adapter that routes delegation
  inference calls through the Bifrost gateway (declarative routing rules,
  failover, circuit breaking, HMAC outbound auth, audit metadata). Returns
  ``ModelBifrostRunnerResult | None``; ``None`` signals Bifrost unavailability
  so the caller may fall back to a legacy direct-HTTP path.

* ``InProcessDelegationRunner`` — synchronous pipeline runner that drives the
  full delegation pipeline (routing → inference → quality gate) in a single
  process using the same handler pure-functions the Kafka-backed pipeline uses
  in production. No Kafka, Postgres, or Docker required. Suitable for local
  delegation from the UserPromptSubmit hook when the runtime socket is
  unavailable. Returns ``ModelDelegationResult``; raises
  ``DelegationRunnerError`` on routing or transport failure.
...
# ---------------------------------------------------------------------------
# Audit event model (Bifrost adapter)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Runner result model (Bifrost adapter)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# DelegationRunnerError (raised by InProcessDelegationRunner)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# DelegationRunner — Bifrost gateway adapter
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Env-based config builder (Bifrost adapter)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Response text extractor (Bifrost adapter)
# ---------------------------------------------------------------------------
...
class InProcessDelegationRunner:
    ...
    Distinct from :class:`DelegationRunner` (Bifrost gateway adapter) — use
    this runner when the runtime socket / Kafka path is unavailable
    (e.g. UserPromptSubmit hook running on a developer laptop without
    Docker). The Bifrost adapter is the production-canonical path.
