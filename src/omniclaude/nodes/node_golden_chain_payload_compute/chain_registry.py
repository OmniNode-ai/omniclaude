...
GOLDEN_CHAIN_METADATA: tuple[ModelChainMetadata, ...] = (
    ...
    ModelChainMetadata(
        ...
        tail_table="delegation_events",
        expected_fields=("correlation_id", "model_name"),
    ),
    ModelChainMetadata(
        name="d1_d2_scaffold",
        head_topic=TopicBase.DELEGATION_INFRA_COMPLETED,
        tail_table="delegation_events",
        expected_fields=("correlation_id", "quality_gate_passed", "response_text"),
    ),
    ModelChainMetadata(
        name="d4_blank_content",
        head_topic=TopicBase.DELEGATION_INFERENCE_RESPONSE,
        tail_table="delegation_events",
        expected_fields=("correlation_id", "response_text", "model_name"),
    ),
    ModelChainMetadata(
    ...
    ModelChainMetadata(
        ...
        head_topic=TopicBase.DELEGATION_REQUEST,
        tail_table="delegation_events",
        expected_fields=("correlation_id", "timestamp"),
    ),
    ModelChainMetadata(
