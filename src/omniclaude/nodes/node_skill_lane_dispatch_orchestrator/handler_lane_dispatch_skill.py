...
"""Canonical handler for the lane_dispatch skill orchestrator."""
...
class HandlerLaneDispatchSkill:
    """Adapter that keeps lane_dispatch on the shared skill dispatch path."""
    ...
    async def handle(self, request: ModelSkillRequest) -> ModelSkillResult:
        ...
        if self._task_dispatcher is None:
            raise RuntimeError("task_dispatcher is required for lane_dispatch")
...
__all__ = ["HandlerLaneDispatchSkill"]
