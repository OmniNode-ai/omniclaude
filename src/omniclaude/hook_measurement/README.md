...
**hooks-off** (the OMN-13244 baseline) and **hooks-on** — across tokens/turn,
...
|---------|------|----------|
| Cost-accounting DB (OMN-10619) | `$ONEX_STATE_DIR/hooks/cost_accounting.db` | per tool-call tokens, USD cost, `session_id`, `is_delegated` |
| PRM trajectory store (OMN-10370) | `$ONEX_STATE_DIR/hooks/logs/post-tool-use-trajectory.jsonl` | per tool-call ordering → inter-call latency |
