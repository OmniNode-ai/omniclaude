...
from agents.lib.enum_generator import (
    EnumGenerator,
    EnumValue,
    GeneratedEnum
)
...
'''
        ...
        # Check details contain the error (OnexError stores details in model.context)
        assert hasattr(error, 'model')
        assert 'details' in error.model.context
        assert 'errors' in error.model.context['details']
        assert any('Missing' in err for err in error.model.context['details']['errors'])
...
'''
        ...
        error = exc_info.value
        assert 'validation failed' in str(error).lower()
        # Check details contain method errors (OnexError stores details in model.context)
        assert hasattr(error, 'model')
        assert 'details' in error.model.context
        assert 'errors' in error.model.context['details']
        error_messages = ' '.join(error.model.context['details']['errors'])
    ...
    def test_generated_enum_source_code_structure(
        self, generator, sample_prd_analysis
    ):
    ...
    def test_generated_enum_code_is_executable(
        self, generator, sample_prd_analysis
    ):
    ...
    def test_generated_enum_json_serialization(
        self, generator, sample_prd_analysis
    ):
    ...
    def test_generated_enum_from_string_method(
        self, generator, sample_prd_analysis
    ):
    ...
    def test_generated_enum_display_name(
        self, generator, sample_prd_analysis
    ):
    ...
    def test_multiple_enum_generation_consistency(
        self, generator, sample_prd_analysis
    ):
    ...
    def test_enum_generation_for_different_node_types(self, generator):
        ...
        for node_type in node_types:
            ...
            result = generator.generate_operation_type_enum(
                prd_analysis=prd_analysis,
                service_name=f'{node_type.lower()}_test'
            )
