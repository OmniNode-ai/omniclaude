                ...
                with patch(
                    ...
                    with patch(
                        ...
                        await logger.log_error(
                            "Error", "message", send_slack_notification=False
                        )
                        await logger.log_success("success", {})

                        # Verify all have same correlation_id
                        assert (
                            mock_tool.call_args.kwargs["correlation_id"]
                            == correlation_id
                        )
                        assert (
                            mock_decision.call_args.kwargs["correlation_id"]
                            == correlation_id
                        )
                        assert (
                            mock_error.call_args.kwargs["correlation_id"]
                            == correlation_id
                        )
                        assert (
                            mock_success.call_args.kwargs["correlation_id"]
                            == correlation_id
                        )
