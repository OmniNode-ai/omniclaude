    ...
    async def recommend_mixins(
        ...
        if self.enable_ml and self.learner and self.learner.is_trained():
            ...
            for mixin_name, confidence, explanation in ml_recommendations:
                ...
                recommendations.append(MixinRecommendation(
    ...
    def _generate_suggestions(
        ...
        if not prediction.compatible:
            ...
            if char_a and char_b:
                ...
                if hooks_a & hooks_b:
                    suggestions.append(
                        f"Consider using composition instead of multiple inheritance "
                        f"to resolve lifecycle hook conflicts"
                    )

                # Check for state modification conflicts
                if char_a.state_modifying and char_b.state_modifying:
                    suggestions.append(
                        f"Both mixins modify state - ensure proper ordering "
                        f"and use transactions if needed"
                    )

                # Check for resource conflicts
                if char_a.resource_intensive and char_b.resource_intensive:
                    suggestions.append(
                        f"Both mixins are resource-intensive - monitor performance "
                        f"and consider caching strategies"
                    )
