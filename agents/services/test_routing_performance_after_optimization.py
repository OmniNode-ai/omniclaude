...
async def run_performance_test(num_requests: int = 50) -> None:
    ...
    logger.info(f"  - Target avg time: <100ms")
    logger.info(f"  - Target P95: <200ms")
    logger.info("=" * 70)
    ...
    logger.info(f"Baseline (before optimization):")
    logger.info(f"  Average: {baseline_avg:.2f}ms")
    logger.info(f"  P95:     {baseline_p95:.2f}ms")
    logger.info("")
    logger.info(f"Current (after optimization):")
...
async def verify_database_logging() -> None:
    ...
    try:
        ...
        if recent_decisions:
            ...
            logger.info(f"Recent routing decisions (last 5 min):")
