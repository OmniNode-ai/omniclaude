...
class CoderTaskInput(BaseModel):
    ...
    context_files: List[str] = Field(
        ...
        description="Absolute paths to related files for context"
    )
    validation_level: Literal["strict", "moderate", "lenient"] = Field(
        default="moderate",
        description="How strictly to validate ONEX compliance"
    )
...
class DebugTaskInput(BaseModel):
    ...
    analysis_depth: Literal["quick", "standard", "deep"] = Field(
        default="standard",
        description="How deeply to analyze the issue"
    )
    hypothesis: str = Field(
        default="",
        description="Initial hypothesis about root cause (optional)"
    )
