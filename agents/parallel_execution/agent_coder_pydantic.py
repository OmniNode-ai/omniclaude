...
# System prompt for ONEX-compliant code generation
ONEX_SYSTEM_PROMPT = """You are an expert ONEX architecture code generator.

**ONEX Architecture Principles:**
- 4-node pattern: Effect (I/O), Compute (pure logic), Reducer (aggregation), Orchestrator (workflow)
- Naming: Node<Name><Type> (e.g., NodePostgreSQLAdapterEffect)
...
1. Follows ONEX naming conventions exactly
2. Uses proper Pydantic models for contracts
3. Implements the specific functionality described
4. Includes proper error handling
5. Has clear documentation
6. Uses intelligence/context provided

**Code Quality Requirements:**
- Use actual implementation logic (not TODO placeholders)
- Include proper type hints
- Follow Python best practices
- Generate complete, runnable code
- Leverage any patterns/examples from context

Generate code that matches the EXACT node name and functionality requested."""
...
Generate complete, high-quality code that solves the specific problem described."""
    ...
    def _assemble_code(self, output: ONEXNodeCode) -> str:
        """Assemble final code from structured output."""
        code_parts = []
        ...
        # Add input model
        code_parts.append(output.input_model_code)
        code_parts.append("")

        # Add output model
        code_parts.append(output.output_model_code)
        code_parts.append("")
