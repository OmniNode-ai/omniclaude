...
def sample_routing_stats():
    ...
    return {
        ...
        "latest_decision": datetime.now(UTC) - timedelta(minutes=5),
        "earliest_decision": datetime.now(UTC) - timedelta(days=7),
    }
...
def sample_hook_stats():
    ...
    return {
        "total_events": 200,
        "unique_sources": 3,
        "latest_event": datetime.now(UTC) - timedelta(minutes=10),
        "earliest_event": datetime.now(UTC) - timedelta(days=7),
    }
...
def test_create_statistics_panel_healthy_system(
    ...
    sample_routing_stats["latest_decision"] = datetime.now(UTC) - timedelta(minutes=5)
    sample_hook_stats["latest_event"] = datetime.now(UTC) - timedelta(minutes=10)
...
def test_create_statistics_panel_stale_system():
    ...
    routing_stats = {
        ...
        "latest_decision": datetime.now(UTC) - timedelta(hours=2),
    }
    hook_stats = {
        "total_events": 50,
        "unique_sources": 2,
        "latest_event": datetime.now(UTC) - timedelta(hours=3),
    }
...
async def test_run_dashboard_watch_mode_keyboard_interrupt():
    ...
    with patch("cli.hook_agent_health_dashboard.asyncpg.connect") as mock_connect:
        ...
        with (
            ...
            patch("cli.hook_agent_health_dashboard.Live"),
        ):
...
def test_format_timestamp_with_microseconds():
    ...
    dt = datetime.now(UTC) - timedelta(seconds=30, microseconds=500000)
    result = format_timestamp(dt)
