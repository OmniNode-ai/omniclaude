...
class DebugLogger:
    ...
    def log_phase_start(self, phase_name: str, details: str | None = None):
    ...
    def log_progress(self, message: str, details: str | None = None):
    ...
    def log_error(self, message: str, details: str | None = None):
        """Log error messages (always shown)."""
        print(f"[DispatchRunner] ✗ {message}", file=sys.stderr)
        if details and self.is_verbose():
            print(f"[DispatchRunner] {details}", file=sys.stderr)

    def log_warning(self, message: str, details: str | None = None):
    ...
    def log_debug(self, message: str, details: str | None = None):
    ...
    def log_startup_banner(
        ...
        if self.is_quiet():
            ...
            print(
                f"[Config] Context:{enable_context} Quorum:{enable_quorum} Interactive:{enable_interactive}",
                file=sys.stderr,
            )
        else:
            print("\n" + "=" * 80, file=sys.stderr)
