...
class LinkSuggester:
            ...
            if len(files) >= 2:  # Entity mentioned in at least 2 files
                ...
                for i, file1 in enumerate(file_list):
                    for file2 in file_list[i + 1 :]:
                        ...
                        if (
                            note2["title"] not in note1["existing_links"]
                            and note1["title"] not in note2["existing_links"]
                        ):
                            ...
                            suggestions.append(
                                {
                                    ...
                                    "type": "entity_mention",
                                    "confidence": len(files)
                                    / 10,  # Simple confidence score
                                }
                            )
        ...
        for file_path, note_data in self.notes.items():
            ...
            for other_path, other_data in self.notes.items():
                ...
                other_title_words = set(
                    re.findall(r"\b\w{4,}\b", other_data["title"].lower())
                )
                ...
                if len(common_words) >= 2:  # At least 2 common significant words
                    ...
                    # Check if files don't already link to each other
                    if (
                        other_data["title"] not in note_data["existing_links"]
                        and note_data["title"] not in other_data["existing_links"]
                    ):
    ...
    def generate_report(self, results, output_file=None):
        ...
        report.append(
            f"Found {len(results['entity_suggestions'])} potential connections"
        )
        report.append("")
        ...
        for entity, suggestions in sorted(entity_groups.items()):
            ...
            for suggestion in suggestions[:5]:  # Top 5 per entity
                report.append(
                    f"- [[{suggestion['title1']}]] ↔ [[{suggestion['title2']}]]"
                )
            report.append("")
        ...
        report.append(
            f"Found {len(results['keyword_suggestions'])} potential connections"
        )
        report.append("")

        # Sort by confidence
        sorted_keywords = sorted(
            results["keyword_suggestions"], key=lambda x: x["confidence"], reverse=True
        )
        ...
        report.append(
            f"Found {len(results['orphaned_notes'])} notes with no connections"
        )
        report.append("")

        # Sort by word count (longer notes first)
        sorted_orphaned = sorted(
            results["orphaned_notes"], key=lambda x: x["word_count"], reverse=True
        )
...
def main():
    ...
    parser.add_argument(
        "--vault", default="/Users/cam/VAULT01", help="Path to Obsidian vault"
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument("--json", help="Output JSON file for programmatic use")
