...
class TagStandardizer:
    ...
    def normalize_tag(self, tag):
        ...
        if "/" in tag and not tag.startswith(
            ("ai/", "client/", "learning/", "status/", "action/")
        ):
    ...
    def analyze_existing_tags(self):
        ...
        for file_path in self.vault_path.rglob("*.md"):
            if any(
                skip_dir in file_path.parts
                for skip_dir in {".obsidian", ".trash", "System_Files", ".git"}
            ):
                continue
...
def main():
    ...
    parser.add_argument(
        "--report", action="store_true", help="Generate analysis report of current tags"
    )
    parser.add_argument(
        "--output",
