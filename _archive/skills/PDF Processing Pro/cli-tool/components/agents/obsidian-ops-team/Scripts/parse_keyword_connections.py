...
def main():
    ...
    for conn in connections:
        ...
        tech_score = len(
            [k for k in keywords_lower if any(tech in k for tech in tech_keywords)]
        )
        framework_score = len(
            [k for k in keywords_lower if any(fw in k for fw in framework_keywords)]
        )
        company_score = len(
            [k for k in keywords_lower if any(comp in k for comp in company_keywords)]
        )
        concept_score = len(
            [k for k in keywords_lower if any(conc in k for conc in concept_keywords)]
        )
