...
def add_link_to_file(file_path, link_to_add, link_text=None):
    ...
    if insert_pos is None or (
        insert_pos > 0 and content[insert_pos - 2 : insert_pos] != "\n\n"
    ):
...
def implement_connections():
        ...
        if current_entity and current_entity in [e.lower() for e in PRIORITY_ENTITIES]:
            ...
            if conn_match:
                ...
                if file1_path and file2_path:
                    ...
                    if add_link_to_file(
                        file1_path, file2, f"Related to {current_entity}"
                    ):
                        connections_made += 1
                    ...
                    if add_link_to_file(
                        file2_path, file1, f"Related to {current_entity}"
                    ):
                        connections_made += 1
                        if current_entity not in connections_by_entity:
                            connections_by_entity[current_entity] = 0
    ...
    for entity, count in sorted(
        connections_by_entity.items(), key=lambda x: x[1], reverse=True
    ):
