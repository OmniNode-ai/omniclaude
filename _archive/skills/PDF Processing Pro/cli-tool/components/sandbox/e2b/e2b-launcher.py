...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python e2b-launcher.py <prompt> [components_to_install] [e2b_api_key] [anthropic_api_key]"
        )
        sys.exit(1)
    ...
    anthropic_api_key = (
        sys.argv[4] if len(sys.argv) > 4 else os.getenv("ANTHROPIC_API_KEY")
    )
    ...
    try:
        ...
        while retry_count < max_retries and sbx is None:
            try:
                if retry_count > 0:
                    print(
                        f"🔄 Retry {retry_count}/{max_retries - 1} - WebSocket connection..."
                    )
                ...
                print("🔄 Extending sandbox timeout to prevent early termination...")
                ...
                if (
                    ...
                    if retry_count < max_retries:
                        print(
                            f"⚠️  WebSocket connection failed (attempt {retry_count}), retrying in 3 seconds..."
                        )
                        time.sleep(3)
                        continue
                    else:
                        ...
                        print(
                            "   • Network/firewall restrictions blocking WebSocket connections"
                        )
        ...
        if components_to_install:
            ...
            if install_result.exit_code != 0:
                print("⚠️  Component installation warnings:")
...
Now, please execute this request and create all necessary files."""
        ...
        print(
            f"🤖 Executing Claude Code with prompt: '{prompt[:50]}{'...' if len(prompt) > 50 else ''}'"
        )
        if agents:
        ...
        if check_result.exit_code == 0:
            ...
            ls_result = sbx.commands.run(
                "ls -la /usr/local/bin/ | grep claude", timeout=5
            )
        ...
        test_write = sbx.commands.run(
            "touch test_write.tmp && rm test_write.tmp", timeout=5
        )
        ...
        print(
            f"🚀 Running command: echo '{display_prompt}' | claude -p --dangerously-skip-permissions"
        )
        ...
        if files_result.stdout.strip():
            ...
            project_dir = (
                f"sandbox-{sbx.sandbox_id[:8]}"  # Use first 8 chars of sandbox ID
            )
            ...
            for file_path in files_to_download:
                ...
                if file_path and not file_path.startswith(
                    "./.npm/"
                ):  # Skip npm cache files
                    try:
                        # Read file content from sandbox
                        file_content = sbx.commands.run(
