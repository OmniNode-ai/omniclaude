...
def run_security_validation():
    ...
    try:
        ...
        with open(security_report_path, encoding="utf-8") as f:
                            ...
                            for info_item in integrity_data["info"]:
                                ...
                                if isinstance(info_item, dict):
                                    ...
                                    if "fullHash" in metadata:
                                        security_lookup[key]["hash"] = metadata[
                                            "fullHash"
                                        ]
                                        break
...
def fetch_download_stats():
    ...
    try:
        ...
        if len(all_downloads) > 0:
            ...
            print(
                f"✅ Fetched and aggregated {len(download_counts)} component download stats"
            )
            ...
            if alt_response.status_code == 200:
                ...
                print(
                    f"✅ Fetched stats for {len(download_counts)} components from download_stats"
                )
                return download_counts
            else:
...
def generate_components_json():
                        ...
                        if os.path.isdir(skill_dir_path):
                            ...
                            if os.path.isfile(skill_file_path):
                                ...
                                try:
                                    ...
                                    print(
                                        f"Warning: Could not read file {skill_file_path}: {e}"
                                    )
                                ...
                                security = security_metadata.get(
                                    security_key,
                                    {
                                        "validated": False,
                                        "valid": None,
            ...
            if os.path.isdir(category_path):
                for file_name in os.listdir(category_path):
                    ...
                    if (
                        ...
                        and (file_name.endswith(".md") or file_name.endswith(".json"))
                                ...
                                try:
                                    ...
                                    if component_type == "mcps":
                                        ...
                                        if "mcpServers" in json_data:
                                            for server_name, server_config in json_data[
                                                "mcpServers"
                                            ].items():
                                                if (
                                                    isinstance(server_config, dict)
                                                    and "description" in server_config
                                                ):
            ...
            if os.path.isdir(language_path):
                ...
                for item in os.listdir(language_path):
                    ...
                    if os.path.isfile(item_path):
                        ...
                        if item == ".claude":
                            recursive_files = scan_directory_recursively(
                                item_path, language_path
                            )
                ...
                if os.path.isdir(examples_path):
                    for framework_dir in os.listdir(examples_path):
                        ...
                        if os.path.isdir(framework_path):
                            ...
                            framework_downloads = download_stats.get(
                                framework_download_key, 0
                            )
    ...
    if os.path.isfile(components_marketplace_path):
        ...
        try:
            with open(components_marketplace_path, encoding="utf-8") as f:
                components_marketplace = json.load(f)
            print(
                f"✅ Loaded metadata for {len(components_marketplace.get('agents', []))} agents"
            )
        except Exception as e:
            print(
                f"⚠️ Error reading components metadata from {components_marketplace_path}: {e}"
            )
        ...
        try:
            ...
            if "plugins" in marketplace_data:
                for plugin in marketplace_data["plugins"]:
                    ...
                    commands_names = [
                        extract_component_with_category(cmd) for cmd in commands_list
                    ]
                    agents_names = [
                        extract_component_with_category(agent) for agent in agents_list
                    ]
                    mcps_names = [
                        extract_component_with_category(mcp) for mcp in mcps_list
                    ]
            ...
            print(
                f"✅ Processed {len(components_data['plugins'])} plugins from marketplace.json"
            )
        except Exception as e:
    ...
    try:
        ...
        for component_type, components in components_data.items():
            ...
            if component_type == "templates":
                languages = len(
                    [t for t in components if t.get("subtype") == "language"]
                )
                frameworks = len(
                    [t for t in components if t.get("subtype") == "framework"]
                )
                ...
                print(
                    f"    • {total_commands} commands, {total_agents} agents, {total_mcps} MCPs"
                )
            ...
            if component_type not in ["templates", "plugins"]:
                validated = len(
                    [
                        c
                        for c in components
                        if c.get("security", {}).get("validated", False)
                    ]
                )
                valid_components = len(
                    [c for c in components if c.get("security", {}).get("valid", False)]
                )
                avg_score = (
                    sum(
                        c.get("security", {}).get("score", 0)
                        for c in components
                        if c.get("security", {}).get("validated", False)
                    )
                    / validated
                ...
                print(
                    f"    • Security: {validated} validated, {valid_components} passed, avg score: {avg_score:.1f}"
                )
