...
def scrape_with_rapidapi_jobs():
    ...
    try:
        ...
        for query in search_queries:
            querystring = {"query": query, "location": "Remote", "num_results": "20"}
            ...
            if response.status_code == 200:
                ...
                for job_data in job_results:
                    ...
                    job = {
                        ...
                        "company_icon": job_data.get(
                            "company_logo",
                            get_company_icon(job_data.get("company_name", "")),
                        ),
            ...
            if res.status == 200:
                try:
                    ...
                    for result in search_results:
                        ...
                        if is_claude_code_related(full_text) and is_job_posting(
                            full_text
                        ):

                            # Extract better job information
                            job_info = extract_job_info_from_serper(
                                title, snippet, link
                            )
...
def extract_job_info_from_serper(title, snippet, link):
    ...
    if any(
        skip in title.lower()
        for skip in ["jobs, employment", "jobs available", "browse", "discover"]
    ):
        return None
...
def extract_company_name_improved(title, snippet, link):
    ...
    for pattern in hiring_patterns:
        ...
        if match:
            ...
            if len(company) > 2 and company not in [
                "View",
                "Join",
                "Apply",
                "Software",
                ...
                if story_id:
                    ...
                    if comment_response.status_code == 200:
                        ...
                        for comment in comments:
                            job = extract_job_from_hn_comment(
                                comment, thread.get("title", "")
                            )
                            if job:
                                jobs.append(job)
...
def extract_location_from_hn_comment(text):
    ...
    if any(
        keyword in text.lower() for keyword in ["remote", "anywhere", "distributed"]
    ):
        return "Remote"
...
def scrape_remote_ok():
    ...
    try:
        ...
        headers = {"User-Agent": "claude-code-templates-job-scraper"}
            ...
            for job_data in data[1:]:  # First item is metadata
                ...
                if is_claude_code_related(description + " " + tags_str):
                    job = {
                        ...
                        "location": (
                            "Remote"
                            if job_data.get("location") == "Worldwide"
                            else job_data.get("location", "Remote")
                        ),
                        "description": truncate_description(
                            job_data.get("description", "")
                        ),
...
def scrape_indie_hackers():
    ...
    try:
        ...
        print("✅ Indie Hackers scraping structure ready")
...
def generate_claude_jobs_json():
    ...
    try:
        ...
        remote_jobs = len(
            [job for job in unique_jobs if "remote" in job["location"].lower()]
        )
