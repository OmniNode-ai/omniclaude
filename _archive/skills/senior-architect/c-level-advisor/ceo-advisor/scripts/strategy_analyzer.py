...
class StrategyAnalyzer:
    ...
    def analyze_strategic_position(self, company_data: dict) -> dict:
        ...
        for pillar, config in self.strategic_pillars.items():
            ...
            results["pillar_analysis"][pillar] = {
                ...
                "factors": self._get_pillar_details(
                    company_data.get(pillar, {}), config["factors"]
                ),
            }
        ...
        results["strategic_options"] = self._generate_strategic_options(
            results["pillar_analysis"], company_data.get("context", {})
        )
        ...
        results["roadmap"] = self._create_strategic_roadmap(
            results["strategic_options"], company_data.get("timeline", 12)
        )
            ...
            details.append(
                {
                    ...
                    "status": (
                        "Strong"
                        if score >= 70
                        else "Adequate" if score >= 40 else "Weak"
                    ),
                }
            )
    ...
    def _generate_strategic_options(
        self, pillar_analysis: dict, context: dict
    ) -> list[dict]:
    ...
    def _assess_strategic_risks(
        self, company_data: dict, strategic_options: list
    ) -> dict:
        ...
        roadmap["milestones"] = [
            {
                ...
                "success_criteria": "Core team hired, processes defined",
            },
            {
                "month": 6,
            ...
            {
                "month": 12,
                "milestone": "Strategic review",
                "success_criteria": "ROI demonstrated, strategy validated",
            },
            {
                "month": 18,
                "milestone": "Scale achievement",
                "success_criteria": "Market position improved, financial targets met",
            },
        ]
        ...
        for pillar, analysis in results["pillar_analysis"].items():
            if analysis["score"] < 50:
                if pillar == "market_position":
                    ...
                    recommendations.append(
                        f"Build {pillar}: Invest in talent and culture transformation"
                    )

        # Based on opportunities
        if (
            results["framework_analysis"]["porter_analysis"]["overall_attractiveness"]
            > 70
        ):
            recommendations.append(
...
def analyze_strategy(company_data: dict) -> str:
    ...
    output.extend(["", "Strategic Options:"])
    ...
    output.extend(["", "Key Recommendations:"])
