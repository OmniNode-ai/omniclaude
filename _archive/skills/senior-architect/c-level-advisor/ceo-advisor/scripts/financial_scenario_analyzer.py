...
class FinancialScenarioAnalyzer:
    def __init__(self):
        ...
        self.growth_models = {
            ...
            "logarithmic": lambda base, rate, period: base
            * (1 + rate * math.log(period + 1)),
            "s_curve": lambda base, rate, period: base
            * (2 / (1 + math.exp(-rate * period))),
        }
    ...
    def analyze_scenarios(self, base_case: dict, scenarios: list[dict]) -> dict:
        ...
        results["recommendation"] = self._generate_recommendation(
            results["scenario_analysis"], results["risk_adjusted_view"]
        )
    ...
    def _summarize_financials(self, financials: dict) -> dict:
        ...
        return {
            ...
            "runway_months": self._calculate_runway(
                financials.get("cash", 0), financials.get("burn_rate", 0)
            ),
        }
    ...
    def _analyze_scenario(self, base_case: dict, scenario: dict) -> dict:
        ...
        for year in range(1, 4):  # 3-year projection
            year_projection = self._project_year(current_state, scenario, year)
    ...
    def _project_year(self, current_state: dict, scenario: dict, year: int) -> dict:
        ...
        revenue = model_func(current_state.get("revenue", 0), growth_rate, year)

        # Scale other metrics
        cogs = revenue * scenario.get("cogs_ratio", 0.3)
        opex = current_state.get("operating_expenses", 0) * (
        ...
        return {
            ...
            "cumulative_cash_flow": current_state.get("cumulative_cash_flow", 0)
            + free_cash_flow,
        }
    ...
    def _calculate_irr(
        self, cash_flows: list[float], initial_investment: float
    ) -> float:
        ...
        for projection in projections:
            ...
            if projection.get("ebitda", 0) > 0:
                ...
                if months == 12:
                    return months
                prev_ebitda = (
                    projections[projection["year"] - 2].get("ebitda", 0)
    ...
    def _calculate_total_return(
        self, projections: list[dict], base_case: dict
    ) -> float:
    ...
    def _perform_sensitivity_analysis(
        self, base_case: dict, scenarios: list[dict]
    ) -> dict:
    ...
    def _calculate_variable_impact(
        self, base_case: dict, variable: str, change: float
    ) -> float:
    ...
    def _generate_recommendation(
        self, scenarios: list[dict], risk_adjusted: dict
    ) -> dict:
        ...
        if best_risk_adjusted["npv"] > 0:
            recommendation["rationale"].append(
                f"Positive NPV of ${best_risk_adjusted['npv']:,.0f}"
            )

        if best_risk_adjusted["irr"] > 0.15:
            recommendation["rationale"].append(
                f"Strong IRR of {best_risk_adjusted['irr']:.1%}"
            )

        if (
            best_risk_adjusted["break_even_month"] > 0
            and best_risk_adjusted["break_even_month"] < 24
        ):
            recommendation["rationale"].append(
                f"Quick path to profitability ({best_risk_adjusted['break_even_month']} months)"
            )
        ...
        if risk_adjusted["standard_deviation"] > risk_adjusted["expected_value"] * 0.5:
            recommendation["risk_mitigation"].append(
                "High variability - consider hedging strategies"
            )
    ...
    example_scenarios = [
        {
            ...
            "growth_rate": 0.5,
            "changes": {"operating_expenses": {"multiply": 1.3}},
            "assumptions": [
        ...
        },
        {
            ...
            "growth_model": "linear",
            "growth_rate": 0.15,
            "changes": {"operating_expenses": {"multiply": 0.9}},
