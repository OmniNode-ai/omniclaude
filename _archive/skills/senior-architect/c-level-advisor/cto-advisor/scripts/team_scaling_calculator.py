...
class TeamScalingCalculator:
    ...
    def calculate_scaling_plan(self, current_state: dict, growth_targets: dict) -> dict:
        ...
        results = {
            ...
            "growth_timeline": self._create_growth_timeline(
        ...
        results["budget_projection"] = self._calculate_budget(
            results["hiring_plan"], current_state.get("location", "US")
        )
    ...
    def _create_growth_timeline(self, current: dict, targets: dict) -> list[dict]:
        ...
        for quarter in range(1, timeline_quarters + 1):
            ...
            max_onboarding = math.ceil(
                current_headcount * 0.25
            )  # 25% growth per quarter max
            ...
            timeline.append(
                {
                    ...
                    - (0.2 * (quarterly_growth / max(current_headcount, 1))),
                }
            )
    ...
    def _generate_hiring_plan(self, current: dict, targets: dict) -> dict:
            ...
            if hires_needed > 0:
                hiring_plan["by_role"][role] = {
                    ...
                    "priority": self._get_role_priority(
                        role, current_roles, target_headcount
                    ),
                }
        ...
        for quarter_data in timeline:
            ...
            hiring_plan["by_quarter"][quarter] = {
                "total_hires": hires,
                "breakdown": self._distribute_quarterly_hires(
                    hires, hiring_plan["by_role"]
                ),
            }
    ...
    def _get_role_priority(
        self, role: str, current_roles: dict, target_size: int
    ) -> int:
    ...
    def _distribute_quarterly_hires(self, total_hires: int, role_needs: dict) -> dict:
        ...
        sorted_roles = sorted(
            role_needs.items(), key=lambda x: x[1]["priority"], reverse=True
        )
    ...
    def _design_team_structure(self, target_headcount: int) -> dict:
        ...
        structure["communication_paths"] = (
            target_headcount * (target_headcount - 1)
        ) // 2
    ...
    def _calculate_budget(self, hiring_plan: dict, location: str) -> dict:
        ...
        if hiring_plan["total_hires_needed"] > 0:
            budget["cost_per_hire"] = (
                budget["total_cost"] / hiring_plan["total_hires_needed"]
            )
    ...
    def _assess_scaling_risks(self, current: dict, targets: dict) -> list[dict]:
        ...
        growth_rate = (targets["target_headcount"] - current["headcount"]) / max(
            current["headcount"], 1
        )
        ...
        if current_size > 0:
            ...
            if growth_rate > 1.0:
                ...
                recommendations.append(
                    "Focus on senior hires first to establish culture"
                )
                recommendations.append(
...
def calculate_team_scaling(current_state: dict, growth_targets: dict) -> str:
    ...
    output.extend(["", "Hiring Priorities:"])
