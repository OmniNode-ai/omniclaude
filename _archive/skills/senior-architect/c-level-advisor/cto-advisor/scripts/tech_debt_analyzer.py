...
class TechDebtAnalyzer:
    ...
    def _prioritize_actions(
        self, category_scores: dict, business_context: dict
    ) -> list:
    ...
    def _estimate_effort(self, actions: list, team_size: int) -> dict:
        ...
        return {
            ...
            "recommended_team_size": max(
                team_size, math.ceil(total_story_points / 200)
            ),
        ...
        for category, scores in results["category_scores"].items():
            if scores["raw_score"] > 70:
                if category == "architecture":
                    recommendations.append("Consider hiring architecture specialist")
                elif category == "security":
                    recommendations.append("Engage security audit firm")
                elif category == "performance":
                    recommendations.append("Implement performance SLA monitoring")

        # Team recommendations
        effort = results.get("estimated_effort", {})
        if (
            effort.get("recommended_team_size", 0)
            > effort.get("total_story_points", 0) / 200
        ):
            recommendations.append(
...
def analyze_technical_debt(system_config: dict) -> str:
    ...
    output.extend(["", "Strategic Recommendations:"])
