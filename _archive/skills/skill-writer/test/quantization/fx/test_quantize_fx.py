...
class TestFuseFx(QuantizationTestCase):
        ...
        with override_quantized_engine("onednn"):
            ...
            for with_bn, with_relu, left_conv, two_conv, use_torch_add in options:
                node_occurrence = {
                    ...
                    ns.call_module(
                        nniq.ConvAddReLU2d if with_relu else nniq.ConvAdd2d
                    ): 1,
                ...
                self.checkGraphModuleNodes(
                    m_ref, expected_node_occurrence=node_occurrence_ref
                )
                m(*example_x)
    ...
    @unittest.skip(
        "Temporarily skipping the test case, will enable after the simple"
        ...
        for use_torch_add in [True, False]:
            ...
            conv_bn_res_relu_config1 = (
                ...
                ._set_pattern_complex_format(
                    (nn.ReLU, (torch.add, MatchAllNode, (nn.BatchNorm2d, nn.Conv2d)))
                )
            ...
            conv_bn_res_relu_config2 = (
                BackendPatternConfig()
                ._set_pattern_complex_format(
                    (nn.ReLU, (operator.add, MatchAllNode, (nn.BatchNorm2d, nn.Conv2d)))
                )
                .set_fuser_method(fuse_conv_bn_relu)
            )
            backend_config = (
    ...
    def test_fusion_pattern_with_multiple_inputs(self):
        ...
        for node in m.graph.nodes:
            if (
                node.op == "call_module"
                and type(named_modules[node.target]) is torch.nn.Conv2d
            ):
                self.assertTrue(
                    len(node.args) == 2,
                    msg="Expecting the fused op to have two arguments",
                )
    ...
    def test_fusion_pattern_with_matchallnode(self):
        ...
        conv_relu_config = BackendPatternConfig((nn.Conv2d, nn.ReLU)).set_fuser_method(
            fuse_conv_relu
        )
        conv_res_relu_config = (
            BackendPatternConfig()
            ._set_pattern_complex_format(
                (nn.ReLU, (torch.add, nn.Conv2d, MatchAllNode))
            )
...
class TestQuantizeFx(QuantizationTestCase):
    ...
    def test_fused_module_qat_swap(self):
        ...
        prepared = prepare_qat_fx(
            model, qconfig_dict, example_inputs=(torch.randn(1, 5),)
        )
        self.assertTrue(
            isinstance(
                getattr(prepared.mods1, "0").tmp, torch.ao.nn.intrinsic.qat.LinearReLU
            )
        )
        ...
        tests = [
            (
                ...
                ns.call_function(
                    ...
                    if is_reference
                    else torch.ops.quantized.conv1d
                ),
            ...
            (
                ...
                ns.call_function(
                    ...
                    if is_reference
                    else torch.ops.quantized.conv3d
                ),
            ...
            (
                ...
                (
                    None
                    if is_reference
                    else ns.call_function(torch.ops.quantized.linear_dynamic)
                ),
            ...
            (
                ...
                ns.call_function(
                    ...
                    if is_reference
                    else torch.ops.quantized.linear
                ),
                ns.call_function(torch.ops.quantized.linear_prepack),
            ),
            (
                ...
                (linear_module_input,),
                (
                    ns.call_module(nnqr.Linear)
                    if is_reference
                    else ns.call_module(nnqd.Linear)
                ),
                None,
            ),
            (
    ...
    def test_conv_linear_reference(self):
        ...
        def _get_keys(prefix, is_dynamic):
            ...
            if not is_dynamic:
                all_keys.extend(
                    [prefix + "." + k for k in ["weight_scale", "weight_zero_point"]]
                )
            return all_keys
            ...
            def checkWeightQParams(model):
                for module_name in ("linear", "conv"):
                    if hasattr(model, module_name):
                        self.assertTrue(
                            hasattr(qr.get_submodule(module_name), "weight_qscheme")
                        )
                        self.assertTrue(
                            hasattr(qr.get_submodule(module_name), "weight_scale")
                        )
                        self.assertTrue(
                            hasattr(qr.get_submodule(module_name), "weight_zero_point")
                        )
                        self.assertTrue(
                            "Reference" in qr.get_submodule(module_name)._get_name()
                        )
        ...
        tests = [
            (
                ...
                ns.call_function(
                    ...
                    if is_reference
                    else torch.ops.quantized.conv_transpose1d
                ),
            ...
            (
                ...
                ns.call_function(
                    ...
                    if is_reference
                    else torch.ops.quantized.conv_transpose2d
                ),
            ...
            (
                ...
                ns.call_function(
                    ...
                    if is_reference
                    else torch.ops.quantized.conv_transpose3d
                ),
            ...
            (
                ...
                (conv_transpose1d_input,),
                ns.call_module(
                    nnqr.ConvTranspose1d if is_reference else nnq.ConvTranspose1d
                ),
                None,
            ),
            (
                ...
                (conv_transpose2d_input,),
                ns.call_module(
                    nnqr.ConvTranspose2d if is_reference else nnq.ConvTranspose2d
                ),
                None,
            ),
            (
                ...
                (conv_transpose3d_input,),
                ns.call_module(
                    nnqr.ConvTranspose3d if is_reference else nnq.ConvTranspose3d
                ),
                None,
            ),
        ]
    ...
    def test_dynamic_quant_weight_observer(self):
        ...
        ref_qparams = (
            weight_obs.calculate_qparams()[0].item(),
            weight_obs.calculate_qparams()[1].item(),
        )
    ...
    def test_linear_bn(self):
        ...
        for quant_type in self.static_quant_types:
            ...
            m_eager = copy.deepcopy(m)
            result_dict = self.checkGraphModeFxOp(
                m, data, quant_type, expected_node=expected_node
            )
            result = result_dict["quantized_output"]
            ...
            if is_name:
                prepare_config = {
                    "standalone_module_name": [
                        ("standalone", None, sm_example_inputs, interface_config, None)
                    ]
                }
            else:
                prepare_config = {
    ...
    def test_qconfig_precedence(self):
        for device in get_supported_device_types():
            ...
            self.assertEqual(
                m_prep.linear.qconfig.activation.p.func,
                global_qconfig.activation.p.func,
            )
            self.assertEqual(
                m_prep.linear.qconfig.weight.p.func, global_qconfig.weight.p.func
            )
            self.assertEqual(
                m_prep.conv.qconfig.activation.p.func,
                object_type_qconfig.activation.p.func,
            )
            self.assertEqual(
                m_prep.conv.qconfig.weight.p.func, object_type_qconfig.weight.p.func
            )
            self.assertEqual(
                m_prep.module_conv1.qconfig.activation.p.func,
                module_name_regex_qconfig.activation.p.func,
            )
            self.assertEqual(
                m_prep.module_conv1.qconfig.weight.p.func,
                module_name_regex_qconfig.weight.p.func,
            )
            self.assertEqual(
                m_prep.module_conv2.qconfig.activation.p.func,
                module_name_qconfig.activation.p.func,
            )
            self.assertEqual(
                m_prep.module_conv2.qconfig.weight.p.func,
                module_name_qconfig.weight.p.func,
            )
    ...
    def test_qconfig_mapping_set_object_type(self):
        ...
        self.assertEqual(
            qconfig_mapping.object_type_qconfigs[torch.nn.Linear], qconfig1
        )
        ...
        self.assertEqual(
            qconfig_mapping.object_type_qconfigs[torch.nn.Linear], qconfig3
        )
        self.assertEqual(qconfig_mapping.object_type_qconfigs[torch.nn.ReLU], qconfig2)
        self.assertEqual(
            _get_object_type_qconfig(qconfig_mapping, torch.nn.Linear, None), qconfig3
        )
        self.assertEqual(
            _get_object_type_qconfig(qconfig_mapping, torch.nn.ReLU, None), qconfig2
        )
        self.assertEqual(
            _get_object_type_qconfig(qconfig_mapping, "nomatch", None), None
        )
    ...
    def test_qconfig_mapping_set_module_name_regex(self):
        ...
        self.assertEqual(
            qconfig_mapping.module_name_regex_qconfigs["foo.*bar"], qconfig1
        )
        ...
        self.assertEqual(
            qconfig_mapping.module_name_regex_qconfigs["foo.*bar"], qconfig3
        )
        self.assertEqual(qconfig_mapping.module_name_regex_qconfigs["foo.*"], qconfig2)
        self.assertEqual(
            _get_module_name_regex_qconfig(qconfig_mapping, "foo123bar", None), qconfig3
        )
        self.assertEqual(
            _get_module_name_regex_qconfig(qconfig_mapping, "foobar", None), qconfig3
        )
        self.assertEqual(
            _get_module_name_regex_qconfig(qconfig_mapping, "foobaz", None), qconfig2
        )
        self.assertEqual(
            _get_module_name_regex_qconfig(qconfig_mapping, "foo", None), qconfig2
        )
        self.assertEqual(
            _get_module_name_regex_qconfig(qconfig_mapping, "nomatch", None), None
        )
    ...
    def test_qconfig_mapping_set_module_name(self):
        ...
        self.assertEqual(qconfig_mapping.module_name_qconfigs["mod2"], qconfig2)
        self.assertEqual(
            _get_module_name_qconfig(qconfig_mapping, "mod1", None), qconfig3
        )
        self.assertEqual(
            _get_module_name_qconfig(qconfig_mapping, "mod2", None), qconfig2
        )
        self.assertEqual(
            _get_module_name_qconfig(qconfig_mapping, "nomatch", None), None
        )
    ...
    def test_qconfig_mapping_set_module_name_object_type_order(self):
        ...
        qconfig_mapping.set_module_name_object_type_order(
            "mod1", torch.nn.Linear, 0, qconfig1
        )
        qconfig_mapping.set_module_name_object_type_order(
            "mod2", torch.nn.ReLU, 1, qconfig2
        )
        ...
        self.assertEqual(
            next(iter(qconfig_mapping.module_name_object_type_order_qconfigs)), key1
        )
        self.assertEqual(
            list(qconfig_mapping.module_name_object_type_order_qconfigs)[1], key2
        )
        self.assertEqual(
            qconfig_mapping.module_name_object_type_order_qconfigs[key1], qconfig1
        )
        self.assertEqual(
            qconfig_mapping.module_name_object_type_order_qconfigs[key2], qconfig2
        )
        self.assertEqual(
            _maybe_adjust_qconfig_for_module_name_object_type_order(
                qconfig_mapping, "mod1", torch.nn.Linear, 0, None
            ),
            qconfig1,
        )
        self.assertEqual(
            _maybe_adjust_qconfig_for_module_name_object_type_order(
                qconfig_mapping, "mod2", torch.nn.ReLU, 1, None
            ),
            qconfig2,
        )
        # Override existing key
        qconfig_mapping.set_module_name_object_type_order(
            "mod1", torch.nn.Linear, 0, qconfig3
        )
        self.assertEqual(len(qconfig_mapping.module_name_object_type_order_qconfigs), 2)
        self.assertEqual(
            next(iter(qconfig_mapping.module_name_object_type_order_qconfigs)), key1
        ...
        self.assertEqual(
            qconfig_mapping.module_name_object_type_order_qconfigs[key1], qconfig3
        )
        self.assertEqual(
            qconfig_mapping.module_name_object_type_order_qconfigs[key2], qconfig2
        )
        ...
        self.assertEqual(
            _maybe_adjust_qconfig_for_module_name_object_type_order(
                qconfig_mapping, "mod123", torch.nn.Linear, 0, None
            ),
            None,
        )
        self.assertEqual(
            _maybe_adjust_qconfig_for_module_name_object_type_order(
                qconfig_mapping, "mod1", torch.nn.Linear, 35, None
            ),
            None,
        )
        self.assertEqual(
            _maybe_adjust_qconfig_for_module_name_object_type_order(
                qconfig_mapping, "mod2", torch.nn.Conv2d, 1, None
            ),
            None,
        )

    def _get_qconfig_dict_for_qconfig_mapping_test(
        self, global_qconfig, qconfig1, qconfig2
    ):
        """
        ...
        with self.assertRaises(ValueError) as context:
            m = prepare_fx(
                m, qconfig_dict, example_inputs=(torch.randn(1, 3, 3, 3),)
            )  # noqa: F821
        self.assertTrue(
    ...
    def test_qconfig_mapping_from_dict(self):
        ...
        qconfig_dict = self._get_qconfig_dict_for_qconfig_mapping_test(
            global_qconfig, qconfig1, qconfig2
        )
        ...
        self.assertEqual(
            qconfig_mapping.object_type_qconfigs,
            OrderedDict(
                {
                    torch.nn.Linear: qconfig1,
                    torch.nn.ReLU: qconfig2,
        ...
        self.assertEqual(
            qconfig_mapping.module_name_regex_qconfigs,
            OrderedDict(
                {
                    "foo.*bar": qconfig1,
                    "foo.*": qconfig2,
        ...
        self.assertEqual(
            qconfig_mapping.module_name_qconfigs,
            OrderedDict(
                {
                    "bazbaz": qconfig1,
                    "borbor": qconfig2,
        ...
        self.assertEqual(
            qconfig_mapping.module_name_object_type_order_qconfigs,
            OrderedDict(
                {
                    ("bazbaz", torch.nn.Linear, 0): qconfig1,
                    ("foofoo", torch.nn.ReLU, 1): qconfig2,
    ...
    def test_prepare_custom_config_set_standalone_module_name(self):
        ...
        prepare_custom_config.set_standalone_module_name(
            ...
            qconfig_mapping,
            example_inputs,
            child_prepare_custom_config,
            backend_config,
        )
        self.assertEqual(
            list(prepare_custom_config.standalone_module_names.keys()), ["module1"]
        )
        self.assertEqual(
            prepare_custom_config.standalone_module_names["module1"], config_entry
        )
    ...
    def test_prepare_custom_config_set_standalone_module_class(self):
        ...
        self.assertTrue(
            self._DummyStandaloneModule
            in prepare_custom_config.standalone_module_classes
        )
        self.assertEqual(
            prepare_custom_config.standalone_module_classes[
                self._DummyStandaloneModule
            ],
            config_entry,
        )
    ...
    def test_prepare_custom_config_set_float_to_observed_mapping(self):
        ...
        prepare_custom_config.set_float_to_observed_mapping(
            self._DummyFloatModule, self._DummyObservedModule, QuantType.STATIC
        )
        self.assertEqual(len(prepare_custom_config.float_to_observed_mapping), 1)
        self.assertEqual(
            list(prepare_custom_config.float_to_observed_mapping.keys()),
            [QuantType.STATIC],
        )
        self.assertEqual(
            len(prepare_custom_config.float_to_observed_mapping[QuantType.STATIC]), 1
        )
        self.assertTrue(
            self._DummyFloatModule
            in prepare_custom_config.float_to_observed_mapping[QuantType.STATIC]
        )
        self.assertEqual(
            prepare_custom_config.float_to_observed_mapping[QuantType.STATIC][
                self._DummyFloatModule
            ],
    ...
    def test_prepare_custom_config_set_non_traceable_module_names(self):
        ...
        self.assertEqual(
            prepare_custom_config.non_traceable_module_names, ["module1", "module2"]
        )
    ...
    def test_prepare_custom_config_set_non_traceable_module_classes(self):
        ...
        prepare_custom_config.set_non_traceable_module_classes(
    ...
    def _get_dummy_prepare_custom_config_dict(self):
        ...
        return {
            ...
            FLOAT_TO_OBSERVED_DICT_KEY: {
                "static": {self._DummyFloatModule: self._DummyObservedModule},
            },
    ...
    def test_prepare_custom_config_from_dict(self):
        ...
        (sm_name, qm1, ei1, pcc1, bcd1) = prepare_custom_config_dict[
            STANDALONE_MODULE_NAME_DICT_KEY
        ][0]
        (sm_class, qm2, ei2, pcc2, bcd2) = prepare_custom_config_dict[
            STANDALONE_MODULE_CLASS_DICT_KEY
        ][0]
        ...
        prepare_custom_config = PrepareCustomConfig.from_dict(
        ...
        self.assertEqual(
            prepare_custom_config.standalone_module_names[sm_name], sm_config_entry1
        )
        self.assertEqual(len(prepare_custom_config.standalone_module_classes), 1)
        self.assertTrue(sm_class in prepare_custom_config.standalone_module_classes)
        self.assertEqual(
            prepare_custom_config.standalone_module_classes[sm_class], sm_config_entry2
        )
        ...
        # Other
        self.assertEqual(
            prepare_custom_config.non_traceable_module_names, ["module2", "module3"]
        )
        self.assertEqual(
            prepare_custom_config.non_traceable_module_classes,
            [self._DummyNonTraceableModule1, self._DummyNonTraceableModule2],
    ...
    def test_prepare_custom_config_to_dict(self):
        ...
        prepare_custom_config_dict[STANDALONE_MODULE_NAME_DICT_KEY][0] = (
            sm_name,
            qm1.to_dict(),
            ei1,
            pcc1.to_dict(),
        ...
        prepare_custom_config_dict[STANDALONE_MODULE_CLASS_DICT_KEY][0] = (
            sm_class,
            qm2.to_dict(),
            ei2,
            pcc2.to_dict(),
    ...
    def test_convert_custom_config_set_observed_to_quantized_mapping(self):
        ...
        self.assertEqual(
            list(convert_custom_config.observed_to_quantized_mapping.keys()),
            [QuantType.STATIC],
        )
        self.assertTrue(
            self._DummyObservedModule
            in convert_custom_config.observed_to_quantized_mapping[QuantType.STATIC]
        )
        self.assertEqual(
            convert_custom_config.observed_to_quantized_mapping[QuantType.STATIC][
                self._DummyObservedModule
            ],
    ...
    def _get_dummy_convert_custom_config_dict(self):
        ...
        return {
            OBSERVED_TO_QUANTIZED_DICT_KEY: {
                "static": {self._DummyObservedModule: self._DummyQuantizedModule},
            },
            PRESERVED_ATTRIBUTES_DICT_KEY: ["attr1", "attr2"],
        }
    ...
    def test_convert_custom_config_from_dict(self):
        ...
        convert_custom_config = ConvertCustomConfig.from_dict(
        ...
        self.assertEqual(
            len(convert_custom_config.observed_to_quantized_mapping[QuantType.STATIC]),
            1,
        )
        self.assertTrue(
            self._DummyObservedModule
    ...
    def test_convert_custom_config_to_dict(self):
        ...
        self.assertEqual(
            convert_custom_config.to_dict(),
            self._get_dummy_convert_custom_config_dict(),
        )
    ...
    def test_remove_qconfig(self):
        ...
        for name, module in m.named_modules():
            self.assertFalse(
                hasattr(module, "qconfig"), "qconfig is not removed for " + name
            )
    ...
    def test_qat_and_script(self):
        ...
        FileCheck().check_count(
            'FakeQuantize = prim::GetAttr[name="', 4, exactly=True
        ).run(scripted.graph)
    ...
    def test_custom_module_class(self):
        ...
        original_m = M().eval()
        original_ref_m = RefM().eval()
        original_ref_m.linear1.weight = torch.nn.Parameter(
            original_m.linear.weight.detach()
        )
        original_ref_m.linear1.bias = torch.nn.Parameter(
            original_m.linear.bias.detach()
        )
        original_ref_m.linear2.weight = torch.nn.Parameter(
            original_m.custom.linear.weight.detach()
        )
        original_ref_m.linear2.bias = torch.nn.Parameter(
            original_m.custom.linear.bias.detach()
        )

        a16_qconfig = QConfig(
            activation=MinMaxObserver.with_args(
                dtype=torch.qint32, quant_min=0, quant_max=65536
            ),
            ...
            if key == "static":
                prepare_custom_config_dict = {
                    "float_to_observed_custom_module_class": {
                        "static": {CustomModule: ObservedCustomModule}
                    }
                }
                convert_custom_config_dict = {
                    "observed_to_quantized_custom_module_class": {
                        "static": {ObservedCustomModule: quantized_module_class}
                    }
                }
            else:
                prepare_custom_config_dict = {
                    "non_traceable_module_class": [CustomModule]
                }
                convert_custom_config_dict = {
                    "observed_to_quantized_custom_module_class": {
                        "dynamic": {CustomModule: quantized_module_class}
                    }
                }
    ...
    def test_custom_module_class_input_has_multiple_users(self):
        ...
        convert_custom_config_dict = {
            "observed_to_quantized_custom_module_class": {
                "static": {ObservedCustomModule: StaticQuantCustomModule}
            }
        }
        m = M().eval()
        example_inputs = (torch.randn(3, 3),)
    ...
    def test_non_traceable_module(self):
        ...
        qconfig_dict = {"": default_qconfig}
        prepare_custom_config_dict = {
            "non_traceable_module_name": ["m1"],
            "non_traceable_module_class": [NonTraceable2],
        }
        m = prepare_fx(
            m,
            qconfig_dict,
        ...
        class M(torch.nn.Module):
            def __init__(self) -> None:
                ...
                self.convs = torch.nn.Sequential(
                    torch.nn.Conv2d(1, 1, 1), torch.nn.Conv2d(1, 1, 1)
                )
    ...
    def test_qparams_buffers(self):
        ...
        self.assertTrue(
            scale_count == 3,
            "Expect each quantized linear op to have a scale in state_dict",
        )
        self.assertTrue(
            zero_point_count == 3,
            "Expect each quantized linear op to have a zero_point in state_dict",
        )
        m(*example_inputs)
        ...
        for attr_name in [
            "mods1_0_input_scale_0",
            "mods1_0_input_zero_point_0",
            "mods1_0_scale_1",
            "mods1_0_zero_point_1",
            "mods1_1_scale_1",
            "mods1_1_zero_point_1",
            "mods2_scale_1",
            "mods2_zero_point_1",
        ]:
    ...
    def test_state_dict(self):
        ...
        # test load
        ref_weight, ref_bias = torch.ops.quantized.conv2d_unpack(
            state_dict["_packed_weight_0"]
        )
    ...
    def test_not_used(self):
        ...
        m = M().eval()
        qconfig_mapping = get_default_qconfig_mapping().set_global(
            float16_static_qconfig
        )
    ...
    def test_qparams_fqn(self):
        ...
        for attr_name in [
            ...
            "mods1_0_input_zero_point_0",
            "mods1_0_scale_0",
            "mods1_0_zero_point_0",
            "mods1_1_scale_0",
            "mods1_1_zero_point_0",
        ]:
            self.assertTrue(hasattr(m, attr_name), attr_name + " not found.")
    ...
    def test_trace_quantize_per_tensor(self):
        ...
        m = M().eval()
        m = prepare_fx(
            m, {"": default_qconfig}, example_inputs=(torch.randn(1, 1, 3, 3),)
        )
        m = convert_fx(m)
    ...
    def test_copy_node_has_shared_actpp_instance(self):
        ...
        for quant_type in self.static_quant_types:
            ...
            occurrence_map = {
                ...
                QuantType.QAT: {ns.call_module(torch.ao.quantization.FakeQuantize): 2},
            }
    ...
    def _test_static_lstm_helper(
        self, model, prepare_node_occurrence, convert_node_occurrence
    ):
        """
        ...
        prepare_custom_config = PrepareCustomConfig().set_float_to_observed_mapping(
            torch.nn.LSTM, torch.ao.nn.quantizable.LSTM
        )
        convert_custom_config = ConvertCustomConfig().set_observed_to_quantized_mapping(
            torch.ao.nn.quantizable.LSTM, torch.ao.nn.quantized.LSTM
        )
        example_inputs = (
            torch.rand(5, 3, 50),
            torch.rand(1, 3, 50),
        ...
        self.checkGraphModuleNodes(
            model, expected_node_occurrence=prepare_node_occurrence
        )
        model(*example_inputs)

        model = convert_fx(model, convert_custom_config=convert_custom_config)
        self.checkGraphModuleNodes(
            model, expected_node_occurrence=convert_node_occurrence
        )
        model(*example_inputs)
    ...
    def test_static_lstm(self):
        ...
        self._test_static_lstm_helper(
            m, prepare_node_occurrence, convert_node_occurrence
        )
    ...
    def test_static_lstm_consume_tuple(self):
        ...
        class ConsumeWholeTuple(nn.Module):
            ...
            def forward(self, inputs: torch.Tensor, h0: torch.Tensor, c0: torch.Tensor):
                ...
                x = self.module_after_lstm(
                    x
                )  # consume tuple (output, (hidden0, hidden1))
                return x
        ...
        self._test_static_lstm_helper(
            m1, prepare_node_occurrence, convert_node_occurrence1
        )
        ...
        self._test_static_lstm_helper(
            m2, prepare_node_occurrence, convert_node_occurrence2
        )
    ...
    def test_static_lstm_with_custom_fixed_qparams(self):
        ...
        qint32_dtype_config = DTypeConfig(
            input_dtype=torch.qint32, output_dtype=torch.qint32
        )
        my_backend_config = get_qnnpack_backend_config()
        for config in my_backend_config.configs:
            if config.pattern in [
                torch.nn.Sigmoid,
                torch.nn.Tanh,
                torch.add,
                torch.mul,
        ...
        class UserObservedLSTM(torch.ao.nn.quantizable.LSTM):
            ...
            def from_float(cls, float_lstm):
                ...
                example_inputs = (
                    torch.rand(5, 3, 50),
                    (torch.rand(1, 3, 50), torch.randn(1, 3, 50)),
                )
        ...
        for node in cell.graph.nodes:
            ...
            expected_scale, expected_zp, expected_dtype = (
                node_name_to_expected_quantize_args[node.name]
            )
    ...
    def test_observer_fqn(self):
        ...
        prepared = prepare_fx(
            model, {"": default_qconfig}, example_inputs=(torch.randn(1, 5))
        )
        name_list = []
    ...
    def test_conv_lowering(self):
        ...
        qconvs = {
            1: nn.quantized.Conv1d,
            2: nn.quantized.Conv2d,
            3: nn.quantized.Conv3d,
        }
        ...
        for dim in range(1, len(convs) + 1):
            ...
            self.checkGraphModuleNodes(
                m, expected_node_occurrence=expected_node_occurrence
            )
    ...
    def test_register_patterns(self):
        ...
        @_register_quant_pattern(
            "dummy_quant2", default_fixed_qparams_range_0to1_observer
        )
        class DummyQuant2:
            pass

        @_register_quant_pattern(
            "dummy_quant3", default_fixed_qparams_range_neg1to1_observer
        )
        class DummyQuant3:
            pass
        ...
        self.assertEqual(
            _DEFAULT_OUTPUT_OBSERVER_MAP["dummy_quant2"],
            default_fixed_qparams_range_0to1_observer,
        )
        self.assertEqual(
            _DEFAULT_OUTPUT_OBSERVER_MAP["dummy_quant3"],
            default_fixed_qparams_range_neg1to1_observer,
        )
        self._assertFixedQParamsFakeQuantizeEqual(
            _DEFAULT_OUTPUT_FAKE_QUANTIZE_MAP["dummy_quant2"],
            default_fixed_qparams_range_0to1_fake_quant,
        )
        self._assertFixedQParamsFakeQuantizeEqual(
            _DEFAULT_OUTPUT_FAKE_QUANTIZE_MAP["dummy_quant3"],
            default_fixed_qparams_range_neg1to1_fake_quant,
        )
        output_fake_quantize_map = get_default_output_activation_post_process_map(
            is_training=True
        )
        output_observer_map = get_default_output_activation_post_process_map(
            is_training=False
        )
        self.assertEqual(
            output_observer_map.get("dummy_quant3"),
            default_fixed_qparams_range_neg1to1_observer,
        )
        self._assertFixedQParamsFakeQuantizeEqual(
            output_fake_quantize_map.get("dummy_quant3"),
            default_fixed_qparams_range_neg1to1_fake_quant,
        )
    ...
    def test_reuse_input_qconfig(self):
        ...
        for M, is_qat in options:
            ...
            example_inputs = (torch.randn(1, 3, 3, 3),)
            m = prepare_fx(
                m, get_default_qconfig_mapping(), example_inputs=example_inputs
            )
            m = convert_fx(m)
            node_list = [
                ns.call_function(torch.quantize_per_tensor),
    ...
    def test_stack_trace_preserved_linear(self):
        ...
        m = M().eval()
        mp = prepare_fx(
            m, get_default_qconfig_mapping(), example_inputs=(torch.randn(1, 1),)
        )
        ...
        self.assertTrue(
            found_stack_trace,
            f"stack trace not found, node: {n.format_node()}, is_reference: True",
        )
        ...
        self.assertTrue(
            found_stack_trace,
            f"stack trace not found, node: {n.format_node()}, is_reference: False",
        )
    ...
    def test_qconfig_dict_setup(self):
        ...
        for func in [get_default_qconfig_mapping, get_default_qat_qconfig_mapping]:
            for backend in backends:
                ...
                m = prepare_fx(
                    m, qconfig_dict, example_inputs=(torch.randn(1, 1, 1, 1))
                )
                for mod in m.modules():
                    if _is_activation_post_process(mod) and mod.dtype == torch.quint8:
                        ...
                        if issubclass(type(mod), FakeQuantize):
                            self.assertEqual(
                                mod.activation_post_process.quant_min, lower_bnd
                            )
                            self.assertEqual(
                                mod.activation_post_process.quant_max, upper_bnd
                            )
                        else:
    ...
    def _validate_qconfig_against_backend_config_constraints(
        ...
        model = prepare_fx(
            model, qconfig_mapping, example_inputs, backend_config=backend_config
        )
        model(*example_inputs)
        ...
        try:
            self.checkGraphModuleNodes(
                model, expected_node_occurrence=expected_node_occurrence
            )
    ...
    def test_backend_config_quantization_range(self):
        ...
        qconfig2 = QConfig(
            activation=MinMaxObserver.with_args(
                quant_min=0, quant_max=63, dtype=torch.quint8
            ),
            weight=MinMaxObserver.with_args(
    ...
    def test_backend_config_scale_min(self):
        ...
        dtype_config = DTypeConfig(
            input_dtype=DTypeWithConstraints(
                dtype=torch.quint8, scale_min_lower_bound=2**-12
            ),
            output_dtype=DTypeWithConstraints(
                dtype=torch.quint8, scale_min_lower_bound=2**-12
            ),
            weight_dtype=DTypeWithConstraints(
                dtype=torch.qint8, scale_min_lower_bound=2**-12
            ),
            bias_dtype=torch.float,
        )
        ...
        qconfig1 = QConfig(
            ...
            weight=MinMaxObserver.with_args(
                dtype=torch.qint8, qscheme=torch.per_tensor_symmetric, eps=2**-12
            ),
        )
        validate_qconfig(qconfig1, satisfies_constraints=True)
        ...
        qconfig2 = QConfig(
            ...
            weight=MinMaxObserver.with_args(
                dtype=torch.qint8, qscheme=torch.per_tensor_symmetric, eps=2**-10
            ),
        )
        ...
        qconfig4 = QConfig(
            activation=MinMaxObserver.with_args(dtype=torch.quint8),
            weight=MinMaxObserver.with_args(
                dtype=torch.qint8, qscheme=torch.per_tensor_symmetric, eps=2**-14
            ),
        )
        validate_qconfig(qconfig4, satisfies_constraints=False)
    ...
    def test_qnnpack_backend_config(self):
        ...
        all_qconfigs: list[tuple[QConfig, str]] = [
            ...
            (
                get_default_qat_qconfig("qnnpack", version=0),
                "default_qat_qnnpack_qconfig_v0",
            ),
            (
                get_default_qat_qconfig("qnnpack", version=1),
                "default_qat_qnnpack_qconfig_v1",
            ),
    ...
    def test_backend_config_check_for_weight_and_bias(self):
        ...
        # make sure this runs
        m = prepare_fx(
            m, qconfig_mapping, example_inputs, backend_config=backend_config
        )
    ...
    def test__convert_to_reference_decomposed_fx_dynamic_quant(self):
        ...
        with override_quantized_engine("qnnpack"):
            m = M().eval()
            qconfig_mapping = get_default_qconfig_mapping("fbgemm").set_object_type(
    ...
    def test__convert_to_reference_decomposed_fx_per_channel_quant(self):
        ...
        m = M().eval()
        qconfig_mapping = get_default_qconfig_mapping("fbgemm").set_object_type(
            F.linear, default_per_channel_qconfig
        )
        example_inputs = (
            torch.randn(1, 5),
            torch.randn(10, 5),
    ...
    def test_match_pattern_with_multiple_args(self):
        ...
        def _get_pattern_configs():
            ...
            backend_pattern_configs.append(
                BackendPatternConfig()
                ._set_pattern_complex_format(
                    (torch.reshape, torch.transpose, MatchAllNode)
                )  # noqa: E131
                .set_observation_type(observation_type)
                .set_dtype_configs(dtype_configs)
        ...
        backend_config = BackendConfig().set_backend_pattern_configs(
            _get_pattern_configs()
        )
    ...
    def test_linear_size_view(self):
        ...
        for use_relu in [False, True]:
            ...
            self.checkGraphModuleNodes(
                quantized_model, expected_node_occurrence=node_occurrence
            )
    ...
    def test_mixed_dtypes(self):
        ...
        class MyModule(torch.nn.Module):
            ...
            def forward(self, x: torch.Tensor):
                ...
                x = self.float_functional.add(
                    linear2, x
                )  # adding two quint8's together
                return x

        def make_qconfig(scale, zp, dtype):
            return QConfig(
                activation=FixedQParamsObserver.with_args(
                    scale=scale, zero_point=zp, dtype=dtype
                ),
        ...
        class Mod(nn.Module):
            def __init__(self, in_channels, out_channels, kernel_size, dimension):
                ...
                self.weight = nn.Parameter(
                    torch.randn(out_channels, in_channels, *kernel_sizes)
                )
    ...
    def test_lowering_functional_conv_transpose_with_kwargs(self):
        ...
        class Mod(nn.Module):
            def __init__(self, in_channels, out_channels, kernel_size, dimension):
                ...
                kernel_sizes = [kernel_size] * self.dim
                self.weight = nn.Parameter(
                    torch.randn(in_channels, out_channels, *kernel_sizes)
                )
    ...
    def test_keep_original_weights(self):
        ...
        qconfig_mapping = QConfigMapping().set_object_type(
            nn.functional.linear, float16_dynamic_qconfig
        )
        prepared_model = prepare_fx(model, qconfig_mapping, example_inputs)
        prepared_model(example_inputs)
        ...
        self.assertTrue(
            "submodule_1_packed_weight_0" in quantized_model.original_weights_lookup
        )
        ...
        self.assertTrue(
            "submodule_2_packed_weight_0" in quantized_model.original_weights_lookup
        )
        torch.testing.assert_close(
        ...
        self.assertTrue(
            "submodule_3_packed_weight_0" in quantized_model.original_weights_lookup
        )
        torch.testing.assert_close(
        ...
        self.assertTrue(
            "submodule_4_packed_weight_0" in quantized_model.original_weights_lookup
        )
        torch.testing.assert_close(
        ...
        self.assertTrue(
            "submodule_5_packed_weight_0" in quantized_model.original_weights_lookup
        )
        torch.testing.assert_close(
        ...
        torch.testing.assert_close(
            quantized_model.original_weights_lookup["_packed_weight_0"][0], model._w
        )
        torch.testing.assert_close(
            quantized_model.original_weights_lookup["_packed_weight_0"][1], None
        )
...
class TestQuantizeFxOps(QuantizationTestCase):
    ...
    def test_linear_module(self):
        with override_quantized_engine("fbgemm"):
            ...
            for quant_type in self.all_quant_types:
                model = LinearModel()
                quantized_module = (
                    nnqd.Linear if quant_type == QuantType.DYNAMIC else nnq.Linear
                )
                ...
                if quant_type in self.static_quant_types:
                    self.assertEqual(
                        result_dict["quantized_output"],
                        result_dict["quantized_reference_output"],
                    )
        ...
        with override_quantized_engine("fbgemm"):
            ...
            for quant_type, use_bias, has_relu, f_relu in options:
                ...
                quant_type_to_prepare_expected_node_occurrence = {
                    ...
                    QuantType.QAT: {
                        ns.call_module(
                            torch.ao.quantization.FusedMovingAvgObsFakeQuantize
                        ): (3 if has_relu else 4),
                    },
                }
                ...
                convert_node_occurrence = {
                    ns.call_function(torch.quantize_per_tensor): (
                        1 if quant_type != QuantType.DYNAMIC else 0
                    ),
                    qlinear_fun: 1,
                    ns.call_method("dequantize"): (
                        num_dequantize if quant_type != QuantType.DYNAMIC else 0
                    ),
                }
                prepare_expected_node_occurrence = (
                ...
                if quant_type != QuantType.DYNAMIC:
                    ...
                    self.assertIn(
                        "_packed_weight_0", result_dict["quantized"].state_dict().keys()
                    )
    ...
    def test_linear_static_fp16(self):
        ...
        for use_bias, has_relu, f_relu, is_reference in options:
            ...
            prepare_node_occurrence = {
                ...
                ns.call_module(torch.ao.quantization.PlaceholderObserver): 3
                + int(use_bias)
                + int(not has_relu),
            }
            ...
            convert_node_occurrence = {
                ...
                ns.call_method("to"): 3
                + int(use_bias)
                + int(not has_relu and is_reference),
                ns.call_method("dequantize"): 3
                + int(use_bias)
                + int(not has_relu and is_reference),
            }
    ...
    def test_quantized_conv_relu(self):
        ...
        for dim, quant_type in options:
            for m in [
                ...
                self.checkGraphModeFxOp(
                    m, self.img_data_dict[dim], quant_type, quantized_nodes[dim]
                )
    ...
    def _test_binary_op_int8_impl(self, binary_op, ibinary_op, quantized_op):
        ...
        for is_inplace, is_scalar, is_reference in options:
            ...
            self.checkGraphModeFxOp(
                BinaryOp(binary_op, ibinary_op, is_inplace, is_scalar),
                data,
                quant_type,
                quantized_node,
                expected_node_list=node_list,
    ...
    def test_bmm(self):
        ...
        custom_qconfig_dict = {
            "object_type": [
                (torch.bmm, float16_static_qconfig),
        ...
        self.checkGraphModeFxOp(
            BinaryOpNonQuantizedInput(torch.bmm, None, False, False),
            data,
            quant_type,
            expected_node_occurrence=node_occurrence,
            custom_qconfig_dict=custom_qconfig_dict,
    ...
    def _test_quantized_add_mul_qat(
        self, model, example_inputs, expected_node_occurrence
    ):
        qconfig_dict = {"": torch.ao.quantization.get_default_qat_qconfig("fbgemm")}
    ...
    def test_qbatch_norm(self):
        ...
        for quant_type, dim, is_reference in options:
            self.checkGraphModeFxOp(
                ...
                self.img_data_dict[dim],
                quant_type,
                quantized_nodes[is_reference][dim],
                is_reference=is_reference,
            )
    ...
    def _test_activation_impl(
        ...
        options = itertools.product(
            [True, False], [True, False], self.static_quant_types, [True, False]
        )
        quantized_nodes = {
            # is_module
            True: {
        ...
        for is_module, is_inplace, quant_type, is_reference in options:
            self.checkGraphModeFxOp(
                M(is_module, is_inplace),
                self.img_data_2d,
                quant_type,
                quantized_nodes[is_module][is_reference],
                is_reference=is_reference,
            )

    def test_hardswish(self):
        self._test_activation_impl(
            nn.Hardswish, F.hardswish, nnq.Hardswish, torch.ops.quantized.hardswish
        )
    ...
    def _test_norm_float16_impl(
        self, float_module, float_op, op_args, data, skip_op_arg_for_functional=False
    ):
        """Test for normalization op, float_op can be torch op or functional op,
        op_args is a list of positional argument for the module/op
        """
        ...
        for is_module, quant_type in options:
            self.checkGraphModeFxOp(
                ...
                data,
                quant_type,
                custom_qconfig_dict=qconfig_dict,
                expected_node_occurrence=node_occurrence,
            )
    ...
    def _test_default_node_quant_handler_ops(
        ...
        is_reference=True,
        node_list=None,
        additional_quant_pattern_dict=None,
    ):
        class M(torch.nn.Module):
        ...
        quant_type = QuantType.STATIC
        prepare_custom_qconfig_dict = {
            "additional_quant_pattern": additional_quant_pattern_dict
        }
        qconfig_dict = {"": qconfig}
    ...
    def test_fixed_qparams_ops_fp16(self):
        ...
        self.checkGraphModeFxOp(
            ...
            data,
            quant_type,
            custom_qconfig_dict=qconfig_mapping,
            expected_node_occurrence=node_occurrence,
            backend_config=backend_config,
        )
    ...
    def test_copy_node_fp32_input(self):
        ...
        m = M().eval()
        m = prepare_fx(
            m, {"": default_reuse_input_qconfig}, example_inputs=(torch.randn(1),)
        )
        m = convert_fx(m)
        # make sure it runs
        m(torch.rand(1))
    ...
    def test_fixed_qparams_ops(self):
        ...
        for eval_mode in [True, False]:
            ...
            if eval_mode:
                self.assertEqual(
                    m_copy(*example_inputs), prepared_copy(*example_inputs)
                )
            # check the correct number of activation_post_process is inserted
            expected_activation_post_process = (
                FixedQParamsObserver if eval_mode else FixedQParamsFakeQuantize
            )
            count_check = {
    ...
    def test_embedding_bag(self):
        class M(torch.nn.Module):
            def __init__(self) -> None:
                ...
                self.emb = torch.nn.EmbeddingBag(
                    num_embeddings=10, embedding_dim=12, include_last_offset=True
                )
        ...
        for dtype in [torch.quint8, torch.quint4x2]:
            ...
            float_qparams_qconfig = QConfig(
                activation=default_placeholder_observer, weight=float_qparams_observer
            )
            self.checkGraphModeFxOp(
                model,
                example_inputs,
    ...
    def _test_rnn_impl(self, qconfigs, M, module_type_strs, module_types, sample_input):
        ...
        for qconfig, module_type_str in options:
            ...
            if (
                ...
                and qconfig is float16_dynamic_qconfig
            ):
                continue
            ...
            model_graph = prepare_fx(
                model_graph, graph_qconfig_dict, example_inputs=(sample_input,)
            )
    ...
    def test_rnn_cell(self):
        ...
        sample_input = torch.tensor(
            [[100, -155], [-155, 100], [100, -155]], dtype=torch.float
        )
        self._test_rnn_impl(
            qconfigs, RNNCellDynamicModel, module_type_strs, module_types, sample_input
        )
    ...
    def test_rnn(self):
        ...
        self._test_rnn_impl(
            qconfigs, RNNDynamicModel, module_type_strs, module_types, sample_input
        )
    ...
    def test_reshape_fp16(self):
        ...
        qconfig_dict = {
            ...
            "": float16_static_qconfig,
            "object_type": [(torch.nn.functional.linear, default_qconfig)],
        }
        backend_config = get_test_only_legacy_native_backend_config()
...
class TestQuantizeFxModels(QuantizationTestCase):
    ...
    def test_prepare_serialize_switch_device_convert(self):
        ...
        for device in ["cuda", "cpu"]:
            for device_after in ["cuda", "cpu"]:
                ...
                model = Net().to(device).eval()
                qconfig_dict = {"": torch.ao.quantization.get_default_qconfig("fbgemm")}
                model_prepared_first = prepare_fx(
                    model, qconfig_dict, example_inputs=(input,)
                )
                model_prepared_second = prepare_fx(
                    model, qconfig_dict, example_inputs=(input,)
                )
    ...
    def _test_model_impl(
        ...
        if eager_quantizable_model is not None:
            ...
            if is_not_tuple_out:
                ...
                if check_with_eager:
                    self.assertEqual(
                        diff_from_eager[mode][name],
                        0,
                        "Result of graph mode quantization and "
                        + "eager mode quantization on model: "
                        + name
                        + " should match. Mode: "
                        + mode
                        + " diff:"
    ...
    def test_torchvision(self):
        ...
        for mode, name in options:
            pretrained = (
                name in quantized_model_list
            )  # load pretrained model to compare with quantized model
            kwargs = {}
            ...
            if name in quantized_model_list:
                eager_quantizable_model = (
                    quantized_models.__dict__[name](
                        pretrained=False, quantize=False, **kwargs
                    )
                    .eval()
                    .float()
                )
            ...
            model = (
                models.__dict__[name](pretrained=pretrained, **kwargs).eval().float()
            )
    ...
    @skipIfNoFBGEMM
    @unittest.skip(
        "TODO: Test is always failing - https://github.com/pytorch/pytorch/issues/54979"
    )
    def test_resnet18_ddp(self):
        ...
        eager_quantizable_model = (
            quantized_models.__dict__[name](pretrained=False, quantize=False)
            .eval()
            .float()
        )  # noqa: F821
        ...
        for device in get_supported_device_types():
            ...
            class EmbeddingBagLinear(torch.nn.Module):
                def __init__(self) -> None:
                    super().__init__()
                    self.emb = torch.nn.EmbeddingBag(
                        num_embeddings=10, embedding_dim=12, mode="sum"
                    )
            ...
            train_indices = [
                [torch.randint(0, 10, (12, 12)), torch.randn((12, 1))] for _ in range(2)
            ]
            eval_output = [[torch.randint(0, 10, (12, 1))]]

            model = EmbeddingBagLinear().train()
            prepared_fx_model = prepare_qat_fx(
                model, qconfig_dict, example_inputs=(train_indices[0][0],)
            )
    ...
    def test_qat_embedding_linear(self):
        for device in get_supported_device_types():
            ...
            qengine = torch.backends.quantized.engine
            qconfig_dict = {
                "": get_default_qat_qconfig(qengine),
    ...
    def test_qat_functional_linear(self, device):
        ...
        prepared = prepare_qat_fx(
            model, custom_qconfig_dict, example_inputs=example_inputs
        )
