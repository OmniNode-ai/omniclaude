...
class TestFxModelReportDetector(QuantizationTestCase):
    ...
    def test_simple_conv(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            optims_str, per_channel_info = (
                per_channel_detector.generate_detector_report(prepared_model)
            )
            ...
            self.assertEqual(
                per_channel_info["conv"]["backend"], torch.backends.quantized.engine
            )
            ...
            self.assertEqual(
                per_channel_info["conv"]["per_channel_quantization_used"], True
            )
    ...
    def test_multi_linear_model_without_per_channel(self):
        ...
        with override_quantized_engine("qnnpack"):
            ...
            self.assertEqual(
                per_channel_info[rand_key]["backend"], torch.backends.quantized.engine
            )
            ...
            for linear_key in per_channel_info.keys():
                ...
                self.assertEqual(
                    module_entry["per_channel_quantization_supported"], True
                )
    ...
    def test_multiple_q_config_options(self):
        ...
        with override_quantized_engine("qnnpack"):
            ...
            for key in per_channel_info.keys():
                ...
                if "fc" in key:
                    self.assertEqual(
                        module_entry["per_channel_quantization_used"], False
                    )
                elif "conv" in key:
                    self.assertEqual(
                        module_entry["per_channel_quantization_used"], True
                    )
                else:
                    raise ValueError(
    ...
    def test_qat_aware_model_example(self):
        ...
        with override_quantized_engine("qnnpack"):
            ...
            per_channel_detector = PerChannelDetector(torch.backends.quantized.engine)
            optims_str, per_channel_info = (
                per_channel_detector.generate_detector_report(model_fp32_prepared)
            )
...
class TestFxModelReportObserver(QuantizationTestCase):
    ...
    def run_model_and_common_checks(self, model, ex_input, num_epochs, batch_size):
        ...
        for _epoch in range(num_epochs):
            ...
            model.apply(
                lambda module: (
                    module.reset_batch_and_epoch_values()
            ...
            self.assertEqual(
                model.obs1.epoch_activation_min, torch.tensor(float("inf"))
            )
            self.assertEqual(
                model.obs1.epoch_activation_max, torch.tensor(float("-inf"))
            )
    ...
    def test_random_epochs_and_batches(self):
        ...
        for index, model in enumerate(models):
            self.run_model_and_common_checks(
                model, inputs[index], num_epochs, num_batches
            )
    ...
    def test_nested_detection_case(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            with model_prep.graph.inserting_before(target_linear):
                ...
                model_prep.graph.create_node(
                    op="call_module", target=pre_obs_fqn, args=target_linear.args
                )
            ...
            with model_prep.graph.inserting_after(target_linear):
                ...
                model_prep.graph.create_node(
                    op="call_module", target=post_obs_fqn, args=(target_linear,)
                )
            ...
            dynam_vs_stat_str, dynam_vs_stat_dict = (
                dynamic_vs_static_detector.generate_detector_report(model_prep)
            )

            # one of the stats should be stationary, and the other non-stationary
            # as a result, dynamic should be recommended
            data_dist_info = [
                dynam_vs_stat_dict[linear_fqn][
                    DynamicStaticDetector.PRE_OBS_DATA_DIST_KEY
                ],
                dynam_vs_stat_dict[linear_fqn][
                    DynamicStaticDetector.POST_OBS_DATA_DIST_KEY
                ],
            ]
...
class TestFxModelReportClass(QuantizationTestCase):
    ...
    def test_constructor(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            model_prep = quantize_fx.prepare_fx(
                model, q_config_mapping, model.get_example_inputs()[0]
            )
            ...
            detector_name_set = {
                detector.get_detector_name() for detector in test_detector_set
            }
            self.assertEqual(
            ...
            self.assertEqual(
                len(model_report.get_observers_of_interest()), num_expected_entries
            )
        ...
        with override_quantized_engine("fbgemm"):
            ...
            for node in prepared_for_callibrate_model.graph.nodes:
                ...
                if (
                    ...
                    and model_report_str_check in node.target
                ):
            ...
            for detector in test_detector_set:
                self.assertTrue(
                    ...
                    in model_report.get_observers_of_interest().keys()
                )

                # get number of entries for this detector
                detector_obs_of_interest_fqns = (
                    model_report.get_observers_of_interest()[
                        detector.get_detector_name()
                    ]
                )
            ...
            with self.assertRaises(ValueError):
                prepared_for_callibrate_model = (
                    model_report.prepare_detailed_calibration()
                )
    ...
    def test_generate_report(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            filled_detector_set = {
                DynamicStaticDetector(),
                PerChannelDetector(torch.backends.quantized.engine),
            }
            ...
            model_prep_full = quantize_fx.prepare_fx(
                model_full, q_config_mapping, example_input
            )
            model_prep_single = quantize_fx.prepare_fx(
                model_single, q_config_mapping, example_input
            )
            ...
            prepared_for_callibrate_model_full = (
                model_report_full.prepare_detailed_calibration()
            )
            prepared_for_callibrate_model_single = (
                model_report_single.prepare_detailed_calibration()
            )
            ...
            for _ in range(num_iterations):
                example_input = torch.tensor(
                    torch.randint(100, (1, 3, 3, 3)), dtype=torch.float
                )
            ...
            modules_observer_cnt, graph_observer_cnt = self.get_module_and_graph_cnts(
                prepared_for_callibrate_model_full
            )
            self.assertEqual(modules_observer_cnt, 0)  # assert no more observer modules
            self.assertEqual(
                graph_observer_cnt, 0
            )  # assert no more observer nodes in graph
    ...
    def test_generate_visualizer(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            prepared_for_callibrate_model, mod_report = (
            ...
            generated_report = mod_report.generate_model_report(
                remove_inserted_observers=False
            )
    ...
    def test_qconfig_mapping_generation(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            self.assertEqual(
                len(qconfig_mapping.module_name_qconfigs), len(mod_reports_by_fqn)
            )
            ...
            prepared = quantize_fx.prepare_fx(
                TwoThreeOps(), qconfig_mapping, example_input
            )
...
class TestFxDetectInputWeightEqualization(QuantizationTestCase):
    ...
    class TwoBlockComplexNet(torch.nn.Module):
        def __init__(self) -> None:
            ...
            self.conv = torch.nn.Conv2d(
                32, 3, kernel_size=(1, 1), stride=(1, 1), padding=(1, 1), bias=False
            )
    ...
    def _get_prepped_for_calibration_model(self, model, detector_set, fused=False):
        ...
        example_input = model.get_example_inputs()[0]
        return _get_prepped_for_calibration_model_helper(
            model, detector_set, example_input, fused
        )
    ...
    def test_input_weight_equalization_determine_points(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            # get tst model and calibrate
            non_fused = self._get_prepped_for_calibration_model(
                self.TwoBlockComplexNet(), detector_set
            )
            fused = self._get_prepped_for_calibration_model(
                self.TwoBlockComplexNet(), detector_set, fused=True
            )
            ...
            for prepared_for_callibrate_model, _mod_report in [non_fused, fused]:
                ...
                node_fqns = {
                    node.target for node in prepared_for_callibrate_model.graph.nodes
                }
                ...
                obs_name_to_find = (
                    InputWeightEqualizationDetector.DEFAULT_PRE_OBSERVER_NAME
                )
                ...
                for module in prepared_for_callibrate_model.modules():
                    ...
                    is_in_include_list = (
                        sum(isinstance(module, x) for x in mods_to_check) > 0
                    )
    ...
    def test_input_weight_equalization_report_gen(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            # prepare the model for calibration
            prepared_for_callibrate_model, model_report = (
                self._get_prepped_for_calibration_model(model, detector_set)
            )
            ...
            input_weight_str, input_weight_dict = generated_report[
                test_input_weight_detector.get_detector_name()
            ]
            ...
            for module_fqn in input_weight_dict:
                ...
                if "block1.linear" in module_fqn:
                    ...
                    ch_axis = block_1_lin_recs[
                        InputWeightEqualizationDetector.CHANNEL_KEY
                    ]
                    ...
                    input_ratio = torch.sqrt(
                        (per_channel_max - per_channel_min) / (global_max - global_min)
                    )
                    ...
                    weight_ratio = torch.sqrt(
                        (per_channel_max - per_channel_min) / (global_max - global_min)
                    )

                    # also get comp stat for this specific layer
                    comp_stat = block_1_lin_recs[
                        InputWeightEqualizationDetector.COMP_METRIC_KEY
                    ]
...
class TestFxDetectOutliers(QuantizationTestCase):
    ...
    def _get_prepped_for_calibration_model(
        self, model, detector_set, use_outlier_data=False
    ):
        r"""Returns a model that has been prepared for callibration and corresponding model_report"""
        ...
        return _get_prepped_for_calibration_model_helper(
            model, detector_set, example_input
        )
    ...
    def test_outlier_detection_determine_points(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            # get tst model and calibrate
            prepared_for_callibrate_model, mod_report = (
                self._get_prepped_for_calibration_model(
            ...
            number_of_obs_found = sum(
                1 if obs_name_to_find in str(node.target) else 0
                for node in prepared_for_callibrate_model.graph.nodes
            )
            self.assertEqual(number_of_obs_found, correct_number_of_obs_inserted)
    ...
    def test_no_outlier_report_gen(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            # get the specific report for input weight equalization
            outlier_str, outlier_dict = generated_report[
                outlier_detector.get_detector_name()
            ]
            ...
            for module_fqn in outlier_dict:
                ...
                self.assertEqual(
                    len(module_dict[OutlierDetector.COMP_METRIC_KEY]), param_size
                )
                self.assertEqual(
                    len(module_dict[OutlierDetector.NUM_BATCHES_KEY]), param_size
                )
    ...
    def test_all_outlier_report_gen(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            outlier_detector = OutlierDetector(
                ratio_threshold=1, reference_percentile=0
            )
    ...
    def test_multiple_run_consistent_spike_outlier_report_gen(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            for module_fqn in outlier_dict:
                ...
                sufficient_batches_info = module_dict[
                    OutlierDetector.IS_SUFFICIENT_BATCHES_KEY
                ]
                ...
                if module_fqn == "linear.0":
                    ...
                    matched_max = sum(
                        val == 3.28e8
                        for val in module_dict[OutlierDetector.MAX_VALS_KEY]
                    )
...
class TestFxModelReportVisualizer(QuantizationTestCase):
    ...
    def _callibrate_and_generate_visualizer(
        self, model, prepared_for_callibrate_model, mod_report
    ):
        r"""
    ...
    def test_get_modules_and_features(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            mod_rep_visualizer: ModelReportVisualizer = (
                self._callibrate_and_generate_visualizer(
    ...
    def test_generate_tables_match_with_report(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            tensor_headers, tensor_table = table_dict[
                ModelReportVisualizer.TABLE_TENSOR_KEY
            ]
            channel_headers, channel_table = table_dict[
                ModelReportVisualizer.TABLE_CHANNEL_KEY
            ]
            ...
            generated_report_keys: set = set(
                mod_rep_visualizer.generated_reports.keys()
            )
    ...
    def test_generate_tables_no_match(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            empty_tables_dict = mod_rep_visualizer.generate_filtered_tables(
                module_fqn_filter="random not there module"
            )

            # test primarily the dict since it has same info as str
            tensor_headers, tensor_table = empty_tables_dict[
                ModelReportVisualizer.TABLE_TENSOR_KEY
            ]
            channel_headers, channel_table = empty_tables_dict[
                ModelReportVisualizer.TABLE_CHANNEL_KEY
            ]
    ...
    def test_generate_tables_single_feat_match(self):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            single_feat_dict = mod_rep_visualizer.generate_filtered_tables(
                feature_filter=OutlierDetector.MAX_VALS_KEY
            )

            # test primarily the dict since it has same info as str
            tensor_headers, tensor_table = single_feat_dict[
                ModelReportVisualizer.TABLE_TENSOR_KEY
            ]
            channel_headers, channel_table = single_feat_dict[
                ModelReportVisualizer.TABLE_CHANNEL_KEY
            ]
...
def _get_prepped_for_calibration_model_helper(
    model, detector_set, example_input, fused: bool = False
):
    r"""Returns a model that has been prepared for callibration and corresponding model_report"""
    # set the backend for this test
    torch.backends.quantized.engine = "fbgemm"
