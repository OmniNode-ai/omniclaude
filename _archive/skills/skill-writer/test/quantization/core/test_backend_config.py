...
class TestBackendConfig(QuantizationTestCase):
    ...
    dtype_config2 = DTypeConfig(
        input_dtype=torch.float16, output_dtype=torch.float, is_dynamic=True
    )
    ...
    def test_dtype_config_from_dict(self):
        self.assertEqual(
            DTypeConfig.from_dict(self.dtype_config_dict1_legacy), self.dtype_config1
        )
        self.assertEqual(
            DTypeConfig.from_dict(self.dtype_config_dict2_legacy), self.dtype_config2
        )
        self.assertEqual(
            DTypeConfig.from_dict(self.dtype_config_dict1), self.dtype_config1
        )
        self.assertEqual(
            DTypeConfig.from_dict(self.dtype_config_dict2), self.dtype_config2
        )
        self.assertEqual(
            DTypeConfig.from_dict(self.dtype_config_dict3), self.dtype_config3
        )
    ...
    def _get_backend_op_config2(self):
        return (
            BackendPatternConfig(torch.add)
            .add_dtype_config(self.dtype_config2)
    ...
    def test_backend_op_config_set_num_tensor_args_to_observation_type(self):
        ...
        self.assertEqual(
            conf._num_tensor_args_to_observation_type,
            self._num_tensor_args_to_observation_type,
        )
    ...
    def test_backend_op_config_from_dict(self):
        ...
        self.assertEqual(
            conf2._num_tensor_args_to_observation_type,
            self._num_tensor_args_to_observation_type,
        )
    ...
    def test_backend_config_set_backend_pattern_config(self):
        ...
        self.assertEqual(
            conf._pattern_complex_format_to_config,
            {
    ...
    def test_backend_config_from_dict(self):
        ...
        self.assertEqual(
            conf._pattern_complex_format_to_config[key1].to_dict(), op_dict1
        )
        self.assertEqual(
            conf._pattern_complex_format_to_config[key2].to_dict(), op_dict2
        )
    ...
    def test_backend_config_to_dict(self):
        ...
        conf = (
            BackendConfig("name1")
            .set_backend_pattern_config(op1)
            .set_backend_pattern_config(op2)
        )
        conf_dict = {
            "name": "name1",
            "configs": [op_dict1, op_dict2],
...
if __name__ == "__main__":
    raise RuntimeError(
        "This _test file is not meant to be run directly, use:\n\n"
