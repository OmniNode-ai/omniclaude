...
class TestObserver(QuantizationTestCase):
    @given(
        qdtype=st.sampled_from(_INT_DTYPES),
    ...
    def test_per_tensor_observers(self, qdtype, qscheme, reduce_range):
        ...
        def _get_ref_params(
            ...
            assert (
                dtype in _INT_DTYPES
            ), f"Not supported dtype: {dtype}, supported dtypes are {_INT_DTYPES}"
...
class TestHistogramObserver(QuantizationTestCase):
    ...
    def test_histogram_observer_same_inputs(self):
        ...
        self.assertEqual(myobs.max_val, 8.0)
        self.assertEqual(myobs.histogram, [13.25, 3.75, 3.0])
    ...
    def test_histogram_observer_update_within_range_succeeds(self):
        ...
        self.assertEqual(
            myobs.histogram, [1.0, 0.0, 1.0, 2.0, 1.0, 0.0, 0.0, 1.0, 1.0, 1.0]
        )
    ...
    def test_fx_qat_convbn_fused_jit_scriptable(self):
        ...
        for qengine in ["fbgemm", "qnnpack"]:
            with override_quantized_engine(qengine):
                ...
                assert isinstance(
                    quantizable_model.conv, torch.ao.nn.intrinsic.qat.ConvBn2d
                )
