...
def _fake_quantize_learnable_per_tensor_affine_grad_reference(
    dY, X, scale, zero_point, quant_min, quant_max, device, dtype
):
...
def _get_tensor_min_max(
    ...
    if not torch.isinf(running_min):
        min_val_tensor = running_min + averaging_const_tensor * (
            min_val_tensor - running_min
        )
    if not torch.isinf(running_max):
        max_val_tensor = running_max + averaging_const_tensor * (
            max_val_tensor - running_max
        )
...
class TestFakeQuantizeOps(TestCase):
    ...
    def _test_learnable_backward_per_tensor(
        self, X, device, scale_base, zero_point_base, dtype=torch.float32
    ):
    ...
    def test_learnable_forward_per_channel_cuda(self):
        ...
        for dtype in [torch.float32, torch.bfloat16]:
            ...
            channel_size = X_base.size(axis)
            scale_base = (
                torch.normal(mean=0, std=1, size=(channel_size,))
                .clamp(1e-4, 100)
                .to(dtype)
            )
            zero_point_base = torch.normal(mean=0, std=128, size=(channel_size,)).to(
                dtype
            )
    ...
    def _test_learnable_backward_per_channel(
        self, X_base, device, scale_base, zero_point_base, axis, dtype=torch.float32
    ):
    ...
    def test_learnable_backward_per_channel_cuda(self):
        ...
        for dtype in [torch.bfloat16, torch.float32]:
            ...
            zero_point_base = torch.randint(0, 10, zero_point_shape, device="cuda").to(
                dtype=dtype
            )
            self._test_learnable_backward_per_channel(
    ...
    @given(
        dtype=st.sampled_from([torch.float, torch.float64, torch.half, torch.bfloat16]),
        device=st.sampled_from(
            ["cpu", "cuda"] if torch.cuda.is_available() else ["cpu"]
        ),
    )
    def test_fake_quantize_per_tensor_affine_inf(self, dtype, device) -> None:
        ...
        ref_result = (
            min(
                quant_max,
                max(quant_min, torch.round(input_tensor / scale) + zero_point),
            )
            - zero_point
        ) * scale
...
class TestFusedObsFakeQuant(TestCase):
    ...
    def test_fused_obs_fake_quant_moving_avg(
        self, device, sampled_dtype, symmetric_quant, use_bool
    ) -> None:
        """
        ...
        if device == "cpu":
            sampled_dtype = "fp32"
        dtype = {"bf16": torch.bfloat16, "fp16": torch.half, "fp32": torch.float32}[
            sampled_dtype
        ]

        in_running_min_ref = out_running_min_ref = torch.tensor(
            float("inf"), dtype=dtype
        )
        in_running_min_op = torch.tensor(float("inf"), dtype=dtype, device=device)
        in_running_max_ref = out_running_max_ref = torch.tensor(
            float("-inf"), dtype=dtype
        )
    ...
    def test_fused_obs_fake_quant_moving_avg_per_channel(
        self, device, symmetric_quant, use_bool
    ) -> None:
        """
        Tests the case where we call the fused_obs_fake_quant op multiple times
        and update the running_min and max of the activation tensors.
