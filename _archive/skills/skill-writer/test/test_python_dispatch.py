...
from torch.library import Library, _scoped_library, fallthrough_kernel, impl
...
$3: f32[] = torch._ops.aten.add.Tensor($1, $2)""",
    ...
    def test_make_subclass_with_modes(self):
        ...
        with Mode():
            with (
                BasicMode()
            ):  # we can't nest two modes that call make_subclass because it only accepts vanilla tensors
                y = x + x
                z = y + y
        self.assertIsInstance(y, ModeTensor)
        self.assertIsInstance(z, ModeTensor)
