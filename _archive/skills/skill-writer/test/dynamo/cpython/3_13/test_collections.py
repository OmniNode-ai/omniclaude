...
class TestNamedTuple(__TestCase):
    ...
    def test_factory(self):
        ...
        self.assertRaises(
            ValueError, namedtuple, "abc%", "efg ghi"
        )  # type has non-alpha char
        self.assertRaises(
            ValueError, namedtuple, "class", "efg ghi"
        )  # type has keyword
        self.assertRaises(
            ValueError, namedtuple, "9abc", "efg ghi"
        )  # type starts with digit

        self.assertRaises(
            ValueError, namedtuple, "abc", "efg g%hi"
        )  # field with non-alpha char
        self.assertRaises(
            ValueError, namedtuple, "abc", "abc class"
        )  # field has keyword
        self.assertRaises(
            ValueError, namedtuple, "abc", "8efg 9ghi"
        )  # field starts with digit
        self.assertRaises(
            ValueError, namedtuple, "abc", "_efg ghi"
        )  # field with leading underscore
        self.assertRaises(
            ValueError, namedtuple, "abc", "efg efg ghi"
        )  # duplicate field
...
class TestOneTrickPonyABCs(ABCTestCase):
    ...
    def test_Generator(self):
        ...
        self.assertRaisesRegex(
            ValueError, "^huhu$", mgen.throw, ValueError, ValueError("huhu")
        )
...
class TestCollectionABCs(ABCTestCase):
    ...
    def test_Mapping(self):
        ...
        self.validate_abstract_methods(
            Mapping, "__contains__", "__iter__", "__len__", "__getitem__"
        )
        with torch._dynamo.error_on_graph_break(False):
    ...
    def test_Sequence(self):
        ...
        self.validate_abstract_methods(
            Sequence, "__contains__", "__iter__", "__len__", "__getitem__"
        )
...
class TestCounter(__TestCase):
    ...
    def test_order_preservation(self):
        ...
        self.assertEqual(
            list(Counter("abracadabra").items()),
            [("a", 5), ("b", 2), ("r", 2), ("c", 1), ("d", 1)],
        )
        ...
        self.assertEqual(
            list(Counter("xyzpdqqdpzyx").items()),
            [("x", 2), ("y", 2), ("z", 2), ("p", 2), ("d", 2), ("q", 2)],
        )

        # Input order dictates elements() order
        self.assertEqual(
            list(Counter("abracadabra simsalabim").elements()),
            [
                "a",
                "a",
                "a",
