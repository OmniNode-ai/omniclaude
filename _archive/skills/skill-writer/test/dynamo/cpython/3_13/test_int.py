    ...
    def test_denial_of_service_prevented_int_to_str(self):
        ...
        if sw_convert.seconds < sw_convert.clock_info.resolution * 2:
            raise unittest.SkipTest(
                '"slow" conversion took only ' f"{sw_convert.seconds} seconds."
            )
...
class PyLongModuleTests(__TestCase):
    ...
    def test_pylong_misbehavior_error_path_to_str(self, mock_int_to_str):
        with support.adjust_int_max_str_digits(20_000):
            big_value = int("7" * 19_999)
            mock_int_to_str.return_value = None  # not a str
            with self.assertRaises(TypeError) as ctx:
                str(big_value)
            self.assertIn("_pylong.int_to_decimal_string did not", str(ctx.exception))
    ...
    def test_pylong_misbehavior_error_path_from_str(self, mock_int_from_str):
        ...
        with support.adjust_int_max_str_digits(20_000):
            ...
            with self.assertRaises(TypeError) as ctx:
                int(big_value)
            self.assertIn("_pylong.int_from_string did not", str(ctx.exception))
