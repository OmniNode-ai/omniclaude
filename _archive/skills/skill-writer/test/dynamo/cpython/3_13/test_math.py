...
class MathTests(__TestCase):
    ...
    @unittest.skipIf(
        sys.platform == "win32" and platform.machine() in ("ARM", "ARM64"),
    ...
    def testCosh(self):
        ...
        self.ftest(
            "cosh(2)-2*cosh(1)**2", math.cosh(2) - 2 * math.cosh(1) ** 2, -1
        )  # Thanks to Lambert
    ...
    def testExp2(self):
        ...
        self.assertEqual(math.exp2(NINF), 0.0)
    ...
    @requires_IEEE_754
    @unittest.skipIf(
        HAVE_DOUBLE_ROUNDING, "fsum is not exact on machines with double rounding"
    )
    def testFsum(self):
        ...
        test_values = [
            ...
            ([1.0 / n for n in range(1, 1001)], float.fromhex("0x1.df11f45f4e61ap+2")),
            (
            ...
            (
                [
                    2.0**n - 2.0 ** (n + 50) + 2.0 ** (n + 52)
                    for n in range(-1074, 972, 2)
                ]
                + [-(2.0**1022)],
        ...
        test_values.append(
            ([terms[i + 1] - terms[i] for i in range(1000)] + [-terms[1000]], -terms[0])
        )
        ...
        self.assertEqual(
            math.fsum(
                [1e100, FloatLike(1.0), -1e100, 1e-100, 1e50, FloatLike(-1.0), -1e50]
            ),
            1e-100,
        )
        self.assertRaises(OverflowError, math.fsum, [1e308, 1e308])
    ...
    def testGcd(self):
        ...
        self.assertEqual(
            gcd(1216342683557601535506311712, 436522681849110124616458784), 32
        )
    ...
    def testHypot(self):
        ...
        for i in range(len(args) + 1):
            self.assertAlmostEqual(
                hypot(*args[:i]), math.sqrt(sum(s**2 for s in args[:i]))
            )
        ...
        self.assertEqual(hypot(0.75, FloatLike(-1.0)), 1.25)
        self.assertEqual(hypot(FloatLike(-1.0), 0.75), 1.25)
        ...
        for n in range(32):
            self.assertTrue(
                math.isclose(hypot(*([fourthmax] * n)), fourthmax * math.sqrt(n))
            )
    ...
    @requires_IEEE_754
    @unittest.skipIf(
        HAVE_DOUBLE_ROUNDING, "hypot() loses accuracy on machines with double rounding"
    )
    ...
    def testDist(self):
        ...
        self.assertEqual(dist((FloatLike(14.0), 1), (2, -4)), 13)
        self.assertEqual(dist((11, 1), (FloatLike(-1.0), -4)), 13)
        self.assertEqual(dist((14, FloatLike(-1.0)), (2, -6)), 13)
        self.assertEqual(dist((14, -1), (2, -6)), 13)
        self.assertEqual(dist((D(14), D(1)), (D(2), D(-4))), D(13))
        self.assertEqual(dist((F(14, 32), F(1, 32)), (F(2, 32), F(-4, 32))), F(13, 32))
        self.assertEqual(
            dist(
        ...
        # Test corner cases
        self.assertEqual(
            dist((13.25, 12.5, -3.25), (13.25, 12.5, -3.25)), 0.0
        ...
        self.assertEqual(  # Handling of moving max to the end
            dist((1.5, 1.5, 0.5), (0, 0, 0)), dist((1.5, 0.5, 1.5), (0, 0, 0))
        )
    ...
    def test_lcm(self):
        ...
        self.assertEqual(
            lcm(1216342683557601535506311712, 436522681849110124616458784),
    ...
    def testSumProd(self):
        ...
        self.assertEqual(sumprod([-1], [1.0]), -1)
        self.assertEqual(sumprod([1.0], [-1]), -1)
    ...
    def testPow(self):
        ...
        self.assertRaises(OverflowError, math.pow, 1e100, 1e100)
    ...
    def testTanh(self):
        ...
        self.ftest(
            "tanh(1)+tanh(-1)", math.tanh(1) + math.tanh(-1), 0, abs_tol=math.ulp(1)
        )
    ...
    def testTanhSign(self):
        ...
        self.assertEqual(math.copysign(1.0, math.tanh(-0.0)), math.copysign(1.0, -0.0))
    ...
    def test_nan_constant(self):
        ...
        self.assertEqual(math.copysign(1.0, math.nan), 1.0)
    ...
    def test_prod(self):
        ...
        self.assertEqual(prod([1.0, F(3, 2)]), 1.5)
        ...
        self.assertEqual(
            prod([2, 3], start=[1, 2]), [1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2]
        )
    ...
    def testPerm(self):
        ...
        for n in range(500):
            for k in range(n + 1) if n < 100 else range(30) if n < 200 else range(10):
                self.assertEqual(perm(n, k), factorial(n) // factorial(n - k))
    ...
    def test_nextafter(self):
        ...
        self.assertEqual(
            math.nextafter(9223372036854775808.0, 0.0), 9223372036854774784.0
        )
        self.assertEqual(
            math.nextafter(-9223372036854775808.0, 0.0), -9223372036854774784.0
        )
        ...
        # around 1.0
        self.assertEqual(
            math.nextafter(1.0, -INF), float.fromhex("0x1.fffffffffffffp-1")
        )
        self.assertEqual(
            math.nextafter(1.0, INF), float.fromhex("0x1.0000000000001p+0")
        )
        self.assertEqual(
            math.nextafter(1.0, -INF, steps=1), float.fromhex("0x1.fffffffffffffp-1")
        )
        self.assertEqual(
            math.nextafter(1.0, INF, steps=1), float.fromhex("0x1.0000000000001p+0")
        )
        self.assertEqual(
            math.nextafter(1.0, -INF, steps=3), float.fromhex("0x1.ffffffffffffdp-1")
        )
        self.assertEqual(
            math.nextafter(1.0, INF, steps=3), float.fromhex("0x1.0000000000003p+0")
        )
    ...
    @unittest.skip(
        "flaky test under torch dynamo"
    )  # works on pytest and crashes on unittest
    @requires_IEEE_754
    def test_ulp(self):
        ...
        self.assertEqual(
            math.ulp(FLOAT_MAX), FLOAT_MAX - math.nextafter(FLOAT_MAX, -INF)
        )
...
class FMATests(__TestCase):
    ...
    def test_fma_overflow(self):
        ...
        with self.assertRaises(OverflowError):
            math.fma(a, b, c)
        self.assertEqual(math.fma(a, b, -c), float.fromhex("0x1.fffffffffffffp+1023"))
    ...
    def test_random(self):
        ...
        for a_hex, b_hex, c_hex, expected_hex in test_values:
            with self.subTest(
                a_hex=a_hex, b_hex=b_hex, c_hex=c_hex, expected_hex=expected_hex
            ):
    ...
    # Custom assertions.
    def assertIsNaN(self, value):
        self.assertTrue(math.isnan(value), msg="Expected a NaN, got {!r}".format(value))
