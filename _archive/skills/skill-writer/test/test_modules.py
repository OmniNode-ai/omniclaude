...
class TestModule(TestCase):
    ...
    def test_memory_format(self, device, dtype, module_info, training):
        is_sm86or80 = device.startswith("cuda") and (
        ...
        def _check_out_mem_format(output, input_mem_format, module_mem_format):
            def inner_check_out_mem_format(output):
                ...
                if d == 4 and (
                    ...
                    self.assertTrue(
                        output.numel() == 0
                        or output.is_contiguous(memory_format=torch.channels_last)
                    )
                elif d == 5 and (
                    ...
                    self.assertTrue(
                        output.numel() == 0
                        or output.is_contiguous(memory_format=torch.channels_last_3d)
                    )
                else:
    ...
    def test_device_ctx_init(self, device, dtype, module_info, training):
        ...
        with torch.device("meta"):
            module_inputs_meta = module_info.module_inputs_func(
                module_info,
                device=None,
                dtype=dtype,
                requires_grad=False,
        ...
        for module_input, module_input_meta in zip(module_inputs, module_inputs_meta):
            ...
            c_args_meta, c_kwargs_meta = (
                module_input_meta.constructor_input.args,
                module_input_meta.constructor_input.kwargs,
            )
            ...
            for p_meta, p_cpu in chain(
                zip(m.parameters(), m_cpu.parameters()),
