...
from torch._inductor.utils import TMA_DESCRIPTOR_SIZE, fresh_cache, get_num_sms
    ...
    def test_no_lookup_table_entry_autotune_modes(self, max_autotune):
        ...
        def validate_choices(choices):
            if max_autotune:
                ...
                assert any(
                    isinstance(c, ExternKernelCaller) for c in choices
                ), "Should have ExternKernelCaller"
                assert any(
                    isinstance(c, TritonTemplateCaller) for c in choices
