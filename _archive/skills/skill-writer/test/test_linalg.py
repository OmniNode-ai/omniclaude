...
class TestLinalg(TestCase):
    ...
    @unittest.skipIf(
        (not TEST_WITH_ROCM) and _get_torch_cuda_version() < (12, 1),
        "Test is fixed on cuda 12.1 update 1.",
    )
    ...
    def test_tf32_offline_tunableop(self, device, dtype):
        ...
        try:
            with self._tunableop_ctx():
                ...
                last_result = torch.cuda.tunable.get_results()
                found_result = find_tunableop_result(
                    last_result, "GemmTunableOp_tf32_NN", "nn_41_41_41_ld_41_41_41"
                )
                self.assertTrue(found_result is not None)

                found_result = find_tunableop_result(
                    last_result, "GemmTunableOp_float_NN", "nn_41_41_41_ld_41_41_41"
                )
                self.assertTrue(found_result is not None)
    ...
    def test_blaslog_tunableop(self, device, dtype):
        ...
        with self._tunableop_ctx():
            ...
            p = mp.Process(
                target=tunableop_matmul, args=(device, dtype, result_filename, False)
            )
            p.start()
            p.join()
    ...
    def test_ops_append_to_existing_file_tunableop(self, device, dtype):
        ...
        with self._tunableop_ctx():
            ...
            initial_lines = [
                l
                for l in initial_content.split("\n")
    ...
    def test_offline_tuning_append_to_existing_file_tunableop(self, device, dtype):
        ...
        with self._tunableop_ctx():
            ...
            seed_lines = ["GemmTunableOp_float_NT,nt_768_1024_512_ld_1024_1024_768"]
            ...
            with open(untuned_filename) as f:
                initial_content = f.read()
            initial_lines = [
                l.strip() for l in initial_content.split("\n") if l.strip()
            ]
            initial_count = len(initial_lines)
    ...
    def test_rowwise_scaled_gemm_numerics_tunableop(self, device, dtype):
        ...
        ref_scaled_mm = torch._scaled_mm(
            matA, matB, scale_a=scaleA, scale_b=scaleB, out_dtype=torch.bfloat16
        )
        ...
        with self._tunableop_ctx():
            ...
            torch.cuda.tunable.tuning_enable(False)
            tuned_default_scaled_mm = torch._scaled_mm(
                matA, matB, scale_a=scaleA, scale_b=scaleB, out_dtype=torch.bfloat16
            )
    ...
    def test_cross_error(self, device):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "input tensor and the written-to tensor refer to a single memory location",
        ):
    ...
    def test_renorm(self, device):
        m1 = torch.randn(
            20, 20, device=device
        )  # big enough to exercise vectorized path
    ...
    def test_ormqr(self, device, dtype):
        ...
        def run_test(batch, m, n, fortran_contiguous):
            ...
            actual = torch.ormqr(
                reflectors, zero_tau, C_right, left=True, transpose=False
            )
            self.assertEqual(C_right, actual)

        batches = [(), (0,), (2,), (2, 1)]
        ns = [5, 2, 0]
        for batch, (m, n), fortran_contiguous in product(
            batches, product(ns, ns), [True, False]
        ):
    ...
    def test_ormqr_errors_and_warnings(self, device, dtype):
        test_cases = [
            ...
            (
                ...
                (5, 5),
                True,
                r"other.shape\[-2\] must be equal to input.shape\[-2\]",
            ),
            (
                (1, 2, 2),
                (2, 2),
                (1, 2, 2),
                True,
                r"batch dimensions of tau to be equal to input.shape\[:-2\]",
            ),
            (
                (1, 2, 2),
                (1, 2),
                (2, 2, 2),
                True,
                r"batch dimensions of other to be equal to input.shape\[:-2\]",
            ),
            (
                (2, 4, 3),
                (2, 2),
                (2, 3, 10),
                True,
                r"torch.ormqr: other.shape\[-2\] must be equal to input.shape\[-2\]",
            ),
            (
                (2, 4, 3),
                (2, 2),
                (2, 3, 10),
                False,
                r"torch.ormqr: other.shape\[-1\] must be equal to input.shape\[-2\]",
            ),
        ]
    ...
    @dtypesIfCUDA(
        *floating_and_complex_types_and(
            torch.half, *[torch.bfloat16] if SM53OrLater else []
        )
    )
    ...
    def test_householder_product(self, device, dtype):
        def generate_reflectors_and_tau(A):
            ...
            if A.numel() > 0:
                ...
                for A_i, reflectors_i, tau_i in zip(
                    A_cpu.contiguous().view(*flattened_batch_shape), reflectors, tau
                ):
                    reflectors_tmp, tau_i[:] = (
                        torch.from_numpy(x) if isinstance(x, np.ndarray) else x
                        for x in np.linalg.qr(A_i, mode="raw")
                    )
        ...
        shapes = [
            ...
            (0, 0, 0),
            (0, 5, 5),
            (0, 5, 3),  # Zero batch dimension tensors
            (2, 5, 5),
            (2, 5, 3),  # 3-dim tensors
            (2, 1, 5, 5),
            (2, 1, 5, 3),
    ...
    def test_householder_product_errors_and_warnings(self, device):
        test_cases = [
            ...
            (
                (10, 6),
                (20,),
                r"input.shape\[-1\] must be greater than or equal to tau.shape\[-1\]",
            ),
            (
                (6, 10),
                (5,),
                r"input.shape\[-2\] must be greater than or equal to input.shape\[-1\]",
            ),
        ]
        ...
        with self.assertRaisesRegex(
            RuntimeError, "tau dtype Int does not match input dtype"
        ):
    ...
    def _test_lobpcg_method(self, device, dtype, method):
        ...
        def test_tracker(worker):
            ...
            self.assertGreaterEqual(rerr.min(), 0.0)
        ...
        self.assertEqual(
            matmul(A, V), mm(matmul(B, V), E.diag_embed()), atol=prec, rtol=0
        )
        ...
        for m, n, k, density in [
            ...
            E, V = lobpcg(A, B=B, k=k, n=n, largest=False)
            self.assertEqual(
                matmul(A, V), matmul(B, mm(V, E.diag_embed())), atol=prec, rtol=0
            )
    ...
    @unittest.skipIf(
        not TEST_SCIPY
        or (TEST_SCIPY and version.parse(scipy.__version__) < version.parse("1.4.1")),
...
"""
    ...
    @dtypesIfCUDA(
        *floating_and_complex_types_and(
            *[torch.bfloat16] if TEST_WITH_ROCM or SM53OrLater else [], torch.half
        )
    )
    @dtypes(
        torch.bfloat16,
    ...
    @dtypesIfCUDA(
        *floating_types_and(*[torch.bfloat16] if TEST_WITH_ROCM or SM53OrLater else [])
    )
    @dtypes(torch.float, torch.double)
    ...
    def _test_addmm_impl(self, func, activation, device, dtype):
        ...
        self._test_addmm_addmv(
            func, V.unsqueeze(0), m1, m2, beta=1, activation=activation
        )
    ...
    def test__int_mm(self, device, k, n, use_transpose_a, use_transpose_b):
        ...
        SM70 = torch.cuda.is_available() and torch.cuda.get_device_capability() == (
            7,
            0,
        )
        SM75 = torch.cuda.is_available() and torch.cuda.get_device_capability() == (
            7,
            5,
        )
    ...
    def test__int4_mm(self, device, m, k, n):
        ...
        for dtype in [torch.bfloat16] + (
            [torch.float16, torch.float32] if device == "cpu" else []
        ):
    ...
    def test__dyn_quant_matmul_4bit(self, device, m, k, n):
        ...
        def dyn_quant_matmul_4bit(a, b_int4pack, q_group, in_features, out_features):
    ...
    def test_mm_bmm_non_memory_dense(self, device):
        ...
        self.assertEqual(
            torch.mm(A_conj, B, out=out), torch.mm(A_conj_physical, B, out=out)
        )
        self.assertEqual(
            torch.mm(A_conj.t(), B, out=out), torch.mm(A_conj_physical.t(), B, out=out)
        )
        ...
        self.assertEqual(
            torch.bmm(Ab_conj, Bb, out=out_b),
            torch.bmm(Ab_conj_physical, Bb, out=out_b),
        )
        self.assertEqual(
            torch.bmm(t_b(Ab_conj), Bb, out=out_b),
            torch.bmm(t_b(Ab_conj_physical), Bb, out=out_b),
        )

        # test broadcasting
        self.assertEqual(
            torch.bmm(Ab_conj, Bb_, out=out_b),
            torch.bmm(Ab_conj_physical, Bb_, out=out_b),
        )
        self.assertEqual(
            torch.bmm(t_b(Ab_conj), Bb_, out=out_b),
            torch.bmm(t_b(Ab_conj_physical), Bb_, out=out_b),
        )
    ...
    def test_baddbmm(self, device, dtype):
        ...
        if not is_supported:
            ...
            t = make_tensor(
                (num_batches, M, O), dtype=dtype, device=device, low=-1, high=1
            )
            self.assertRaisesRegex(
                RuntimeError,
                "type|Type|not implemented|CUBLAS_STATUS_NOT_SUPPORTED",
    ...
    def test_pinverse(self, device, dtype):
        ...
        def run_test(M):
            ...
            if M.numel() > 0:
                ...
                self.assertEqual(
                    np.matmul(M_, MPI_), np.matmul(M_, MPI_).swapaxes(-2, -1).conj()
                )
                self.assertEqual(
                    np.matmul(MPI_, M_), np.matmul(MPI_, M_).swapaxes(-2, -1).conj()
                )
            else:
                self.assertEqual(
        ...
        for sizes in [(5, 5), (3, 5, 5), (3, 7, 5, 5)]:
            ...
            self.assertEqual(
                ...
                atol=1e-7,
                rtol=0,
                msg="pseudo-inverse for invertible matrix",
            )
    ...
    def test_linalg_matrix_exp_utils(self, device, dtype):
        ...
        def run_test(coeff_shape, data_shape):
            ...
            res2 = (
                x.unsqueeze(0) * coeffs.view(*coeff_shape, *([1] * len(data_shape)))
            ).sum(1)
    ...
    def test_slogdet(self, device, dtype):
        ...
        def run_test(matsize, batchdims, mat_chars):
            ...
            if num_matrices != 0:
                for idx in range(num_matrices):
                    ...
                    if mat_chars[mat_type] == "hermitian":
                        list_of_matrices.append(
                            random_hermitian_matrix(matsize, dtype=dtype, device=device)
                        )
                    elif mat_chars[mat_type] == "hermitian_psd":
                        list_of_matrices.append(
                            random_hermitian_psd_matrix(
                                matsize, dtype=dtype, device=device
                            )
                        )
                ...
                full_tensor = torch.randn(
                    *batchdims, matsize, matsize, dtype=dtype, device=device
                )
        ...
        for matsize, batchdims in itertools.product([0, 3, 5], [(0,), (3,), (5, 3)]):
            ...
            run_test(
                matsize,
                batchdims,
                mat_chars=["hermitian", "hermitian_pd", "hermitian_psd"],
            )
    ...
    def test_slogdet_errors_and_warnings(self, device, dtype):
        ...
        with self.assertRaisesRegex(
            RuntimeError, r"Low precision dtypes not supported"
        ):
            torch.linalg.slogdet(a)
    ...
    def test_det_logdet_slogdet(self, device, dtype):
        ...
        def test_single_det(M, target, desc):
            ...
            self.assertEqual(
                linalg_sdet * linalg_logabsdet.exp(),
                target_sdet * target_logabsdet.exp(),
                atol=1e-6,
                rtol=0,
                msg=f"{desc} (linalg_slogdet)",
            )
            ...
            if sdet.item() < 0:
                self.assertTrue(
                    logdet.item() != logdet.item(), f"{desc} (logdet negative case)"
                )
            else:
                self.assertEqual(
                    logdet.exp(),
                    ...
                    atol=1e-6,
                    rtol=0,
                    msg=f"{desc} (logdet non-negative case)",
                )
            ...
            for x in [0, 2, 4]:
                for scale in [-2, -0.1, 0, 10]:
                    if scale > 0:
                        ...
                        target = torch.zeros_like(ref_M_sdet), torch.full_like(
                            ref_M_logabsdet, -inf
                        )
                    else:
            ...
            for x1, x2 in [(0, 3), (4, 1), (3, 2)]:
                ...
                for scale1, scale2 in [(0.3, -1), (0, 2), (10, 0.1)]:
                    ...
                    if det_scale > 0:
                        ...
                        target = ref_M_sdet.neg(), ref_M_logabsdet + math.log(
                            -det_scale
                        )
    ...
    def test_cholesky_inverse(self, device, dtype):
        ...
        shapes = (0, 3, 5)
        batches = ((), (0,), (3,), (2, 2))
        for shape, batch, upper, contiguous in list(
            itertools.product(shapes, batches, (True, False), (True, False))
        ):
    ...
    def test_cholesky_inverse_errors_and_warnings(self, device, dtype):
        ...
        if self.device_type == "cpu":
            with self.assertRaisesRegex(
                torch.linalg.LinAlgError,
                r"cholesky_inverse: The diagonal element 2 is zero",
            ):
                torch.cholesky_inverse(a)
    ...
    def test_tensordot(self, device):
        ...
        for res_ndim in range(5):
            ...
            res_numpy = torch.from_numpy(
                np.tensordot(a.cpu().numpy(), b.cpu().numpy(), [a_dims, b_dims])
            )
    ...
    def test_ldl_factor(self, device, dtype):
        ...
        def run_test(shape, batch, hermitian):
            A = random_hermitian_pd_matrix(shape, *batch, dtype=dtype, device=device)
            actual_factors, actual_pivots, info = torch.linalg.ldl_factor_ex(
            ...
            if TEST_SCIPY:
                ...
                if expected_pivots.ndim > 1:
                    permuted_expected_L = np.stack(
                        [
                            expected_L[i][expected_pivots[i], :]
                            for i in range(expected_pivots.shape[0])
                        ]
                    )
                else:
        ...
        magma_254_available = self.device_type == "cuda" and _get_magma_version() >= (
            2,
            5,
            4,
        )
        hermitians = (
            (True, False)
            if dtype.is_complex and (self.device_type == "cpu" or magma_254_available)
            else (False,)
        )
    ...
    def test_ldl_solve(self, device, dtype):
        ...
        batches = ((), (4,), (2, 2))
        nrhss = (1, 7)
        for shape, batch, nrhs, hermitian in itertools.product(
            shapes, batches, nrhss, hermitians
        ):
    ...
    @onlyCUDA
    @unittest.skipIf(
        not blaslt_supported_device(), "blasLt not supported on current device"
    )
    @setBlasBackendsToDefaultFinally
    ...
    def test_triu_tril_large_matrix_64bit(self, device):
        ...
        causal_mask = torch.full(
            (q_len, q_len), float("-inf"), device=device, dtype=torch.float32
        )
        causal_mask.triu_(1)
