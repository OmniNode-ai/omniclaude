...
class SerializationMixin:
    ...
    def test_serialization_map_location(self):
        ...
        if torch.mtia.is_available():
            check_map_locations(
                mtia_0_map_locations, torch.float, torch.device("mtia", 0)
            )
            check_map_locations(
                mtia_last_map_locations,
                torch.float,
...
class TestSerialization(TestCase, SerializationMixin):
    ...
    def test_weights_only_blocked_func_error_msg(self):
        ...
        data = {
            ...
            "b": datetime.datetime(
                2025, 1, 1, 12, 0, tzinfo=zoneinfo.ZoneInfo(key="UTC")
            ),
        }
        with tempfile.NamedTemporaryFile() as f:
            torch.save(data, f)
            ...
            with torch.serialization.safe_globals(
                [datetime.datetime, getattr, zoneinfo.ZoneInfo]
            ):
                with self.assertRaisesRegex(
                    UnpicklingError, ".*_unpickle.*zoneinfo.ZoneInfo.*"
                ):
                    torch.load(f)
