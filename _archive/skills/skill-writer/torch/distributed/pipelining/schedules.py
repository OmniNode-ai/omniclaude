...
from .microbatch import TensorChunkSpec, merge_chunks, split_args_kwargs_into_chunks
...
BACKWARD_INPUT, BACKWARD_WEIGHT, and OVERLAP_F_B are supported."
    ...
    def _dump_csv(self, filename: str, format: str = "compute_comms"):
        ...
        if format == "compute_only":
            assert (
                self.pipeline_order is not None
    ...
    def _assert_unsharded(self, stage: _PipelineStageBase):
        ...
        if stage_uses_fsdp:
            ...
            assert (
                stage_idx in self.unsharded_stages
        ...
        def _perform_action(action: _Action) -> None:
            ...
            if comp_type == SEND_F:
                ...
                assert (
                    stage_idx,
                    mb_index,
                ) not in self.bwd_recv_ops, f"Recv twice for {stage_idx=} {mb_index=} without executing backward"
                ...
                if stage_uses_fsdp:
                    ...
                    assert (
                        stage_idx not in self.unshard_ops
