...
SM89OrLater = LazyVal(
    lambda: torch.cuda.is_available() and torch.cuda.get_device_capability() >= (8, 9)
)
SM90OrLater = LazyVal(
    lambda: torch.cuda.is_available() and torch.cuda.get_device_capability() >= (9, 0)
)
SM100OrLater = LazyVal(
    lambda: torch.cuda.is_available() and torch.cuda.get_device_capability() >= (10, 0)
)
SM120OrLater = LazyVal(
    lambda: torch.cuda.is_available() and torch.cuda.get_device_capability() >= (12, 0)
)
...
IS_JETSON = LazyVal(
    lambda: torch.cuda.is_available()
    and (torch.cuda.get_device_capability() in [(7, 2), (8, 7)] or IS_THOR)
)
IS_SM89 = LazyVal(
    lambda: torch.cuda.is_available() and torch.cuda.get_device_capability() == (8, 9)
)
...
def evaluate_gfx_arch_within(arch_list):
    ...
    effective_arch = os.environ.get(
        "PYTORCH_DEBUG_FLASH_ATTENTION_GCN_ARCH_OVERRIDE", gcn_arch_name
    )
...
PLATFORM_SUPPORTS_CUDNN_ATTENTION: bool = LazyVal(
    lambda: evaluate_platform_supports_cudnn_attention()
)
...
PLATFORM_SUPPORTS_GREEN_CONTEXT: bool = LazyVal(
    lambda: evaluate_platform_supports_green_context()
)
...
def _create_scaling_models_optimizers(
    ...
    with torch.no_grad():
        for c, s in zip(
            mod_control.parameters(), mod_scaling.parameters(), strict=True
        ):
            s.copy_(c)
