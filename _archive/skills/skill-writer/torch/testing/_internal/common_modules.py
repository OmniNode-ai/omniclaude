        ...
        for module_info in self.module_info_list:
            ...
            for training, dtype in product(training_flags, dtypes):
                ...
                try:
                    ...
                    if (
                        self.skip_if_dynamo
                        and not torch.testing._internal.common_utils.TEST_WITH_TORCHINDUCTOR
                    ):
...
def module_inputs_torch_nn_KLDivLoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
...
def module_inputs_torch_nn_NLLLoss(
    ...
    cases: list[tuple[str, dict]] = [
        ...
        (
            ...
            {"weight": make_weight(4).abs(), "ignore_index": -1},
        ),
    ]
    ...
    for desc, constructor_kwargs in cases:
        ...
        module_inputs.append(
            ModuleInput(
                ...
                forward_input=FunctionInput(
                    make_input((2, 4, 5, 5)),
                    torch.empty(2, 5, 5, device=device)
                    .uniform_()
                    .mul(4)
                    .floor()
                    .long(),
        ...
        module_inputs.append(
            ModuleInput(
                ...
                forward_input=FunctionInput(
                    ...
                    torch.empty(2, 5, 5, 2, 2, device=device)
                    .uniform_()
                    .mul(4)
                    .floor()
                    .long(),
    ...
    for desc, constructor_kwargs in cases:
        ...
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(
                    make_input(4, 3), make_target(4, 3), var_input
                ),
                desc=desc,
...
def module_inputs_torch_nn_PoissonNLLLoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    def poissonnllloss_reference_fn(
        i, t, log_input=True, full=False, reduction="mean", eps=1e-8
    ):
        if log_input:
        ...
        if full:
            result += (
                t.mul(t.log()) - t + 0.5 * (2.0 * math.pi * t).log()
            ).masked_fill(t <= 1, 0)
    ...
    for desc, constructor_kwargs in cases:
        ...
        input = (
            make_input((2, 3, 4, 5))
            if log_input
            else make_input((2, 3, 4, 5)).abs().add(0.001)
        )
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(
                    input, make_target((2, 3, 4, 5)).floor_().abs_()
                ),
                desc=desc,
                reference_fn=reference_fn,
            )
...
def module_inputs_torch_nn_MSELoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    for desc, constructor_kwargs in cases:
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(
                    make_input((2, 3, 4, 5)), make_target((2, 3, 4, 5))
                ),
                desc=desc,
        ...
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(make_input(()), make_target(())),
                desc=f"{desc}_scalar",
                reference_fn=partial(mse_loss_reference_fn, **constructor_kwargs),
            )
        )
...
def module_inputs_torch_nn_CosineEmbeddingLoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
...
def module_inputs_torch_nn_Softplus(
    ...
    return [
        ...
        ModuleInput(
            constructor_input=FunctionInput(2),
            forward_input=FunctionInput(make_input((10, 20))),
            reference_fn=lambda m, p, i: 1.0 / 2.0 * torch.log1p(torch.exp(2 * i)),
            desc="beta",
        ),
        ModuleInput(
...
def module_inputs_torch_nn_L1Loss(
    ...
    return [
        ModuleInput(
            ...
            / i.numel()
            * sum((a - b).abs().sum() for a, b in zip(i, t, strict=True)),
        ),
        ModuleInput(
            constructor_input=FunctionInput(),
...
def module_inputs_torch_nn_SmoothL1Loss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    for desc, constructor_kwargs in cases:
        ...
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(make_input((5, 10)), make_input((5, 10))),
                desc=desc,
                reference_fn=reference_fn,
            )
        )
...
def module_inputs_torch_nn_BCELoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    module_inputs.append(
        ModuleInput(
            ...
            forward_input=FunctionInput(
                make_input((), low=1e-2, high=1 - 1e-2), make_target(()).gt(0).to(dtype)
            ),
            desc="scalar_weight",
...
def module_inputs_torch_nn_BCEWithLogitsLoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    def bce_withlogitsloss_reference_fn(m, p, i, t, reduction="mean", weight=None):
        ...
        result = (
            ...
            .mul_(i)
            .add_(max_val)
            .add_((-max_val).exp_().add_((-i - max_val).exp_()).log_())
        )
        ...
        module_inputs.append(
            ModuleInput(
                ...
                reference_fn=partial(
                    bce_withlogitsloss_reference_fn, **constructor_kwargs
                ),
            )
        )
...
def module_inputs_torch_nn_CrossEntropyLoss(
    ...
    for reduction, (desc, constructor_kwargs) in product(reductions, cases):
        ...
        def reference_fn(
            m, p, i, t, reduction=reduction, constructor_kwargs=constructor_kwargs
        ):
            return cross_entropy_loss_reference(
                i, t, reduction=reduction, **constructor_kwargs
            )

        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(
                    reduction=reduction, **constructor_kwargs
                ),
                forward_input=FunctionInput(
                    make_input((2, 3, 5, 5)), make_target((2, 5, 5), low=0, high=3)
                ),
        ...
        module_inputs.append(
            ModuleInput(
                ...
                forward_input=FunctionInput(
                    make_input((2, 3, 5)), make_target((2, 5), low=0, high=3)
                ),
        ...
        module_inputs.append(
            ModuleInput(
                ...
                forward_input=FunctionInput(
                    make_input((2, 3)), make_target((2), low=0, high=3)
                ),
        ...
        if constructor_kwargs.get("ignore_index", None) is None:
            ...
            module_inputs.append(
                ModuleInput(
                    ...
                    forward_input=FunctionInput(
                        make_input((5, 3, 4)), make_input((5, 3, 4)).softmax(dim=1)
                    ),
            ...
            module_inputs.append(
                ModuleInput(
                    ...
                    forward_input=FunctionInput(
                        make_input((5, 3)), make_input((5, 3)).softmax(dim=1)
                    ),
            ...
            module_inputs.append(
                ModuleInput(
                    ...
                    forward_input=FunctionInput(
                        make_input((3,)), make_target((), low=0, high=3)
                    ),
...
def module_inputs_torch_nn_CTCLoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    for target_dtype, (desc, constructor_kwargs) in product(target_dtypes, cases):
        ...
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(
                    make_input((50, 3, 15)).log_softmax(2),
        ...
        module_inputs.append(
            ModuleInput(
                ...
                forward_input=FunctionInput(
                    make_input((50, 3, 15)).log_softmax(2),
                    make_target(
                        (30 + 25 + 20,), dtype=target_dtype, low=low, high=high
                    ),
                    (50, 50, 50),
                    (30, 25, 20),
                ),
...
def module_inputs_torch_nn_HingeEmbeddingLoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    for desc, constructor_kwargs in cases:
        ...
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(
                    make_input(()), make_target(()).gt(0).to(dtype).mul_(2).sub_(1)
                ),
                desc=f"scalar_{desc}",
                reference_fn=reference_fn,
            )
...
def module_inputs_torch_nn_HuberLoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
...
def module_inputs_torch_nn_RMSNorm(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    def rms_norm_reference_fn(m, p, i):
        ...
        result = upcasted_i * torch.rsqrt(
            upcasted_i.pow(2).mean(dim=dims, keepdim=True) + m.eps
        )
        if weight is not None:
            result *= weight
...
def module_inputs_torch_nn_LPPool3d(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    for desc, constructor_kwargs in cases:
        ...
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(
                    make_input((50,)), make_input((50,)), make_target((50,)).sign()
                ),
                desc=desc,
                reference_fn=reference_fn,
            )
...
def module_inputs_torch_nn_MultiLabelMarginLoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    for desc, constructor_kwargs in cases:
        ...
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(
                    make_input((10,)), make_target((10), low=0, high=10)
                ),
                desc=f"1d_{desc}",
                reference_fn=reference_fn,
            )
        ...
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(
                    make_input((5, 10)), make_target((5, 10), low=0, high=10)
                ),
                desc=desc,
                reference_fn=reference_fn,
            )
...
def module_inputs_torch_nn_MultiMarginLoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    for desc, constructor_kwargs in cases:
        ...
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(
                    make_input((5, 10)), make_target((5), low=0, high=10)
                ),
                desc=desc,
                reference_fn=reference_fn,
            )
...
def module_inputs_torch_nn_MultiLabelSoftMarginLoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    def multilabelsoftmargin_loss_reference_fn(
        m, p, i, t, reduction="mean", weight=None
    ):
    ...
    for desc, constructor_kwargs in cases:
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(
                    make_input((5, 10)), make_target((5, 10), low=0, high=2)
                ),
                desc=desc,
                reference_fn=partial(
                    multilabelsoftmargin_loss_reference_fn, **constructor_kwargs
                ),
            )
        )
...
def module_inputs_torch_nn_SoftMarginLoss(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
    ...
    for desc, constructor_kwargs in cases:
        ...
        module_inputs.append(
            ModuleInput(
                constructor_input=FunctionInput(**constructor_kwargs),
                forward_input=FunctionInput(
                    make_input((5, 5)), make_target((5, 5)).sign()
                ),
                desc=desc,
                reference_fn=reference_fn,
            )
...
def module_inputs_torch_nn_TransformerEncoderLayer(
    ...
    for (
        ...
        attn_masks, key_padding_masks, (True, False), (True, False), (True, False)
    ):
        samples.append(
            ModuleInput(
                constructor_input=FunctionInput(
...
def module_inputs_torch_nn_TransformerDecoderLayer(
    ...
    for (
        ...
        if tgt_key_padding_mask is not None:
            memory_key_padding_mask, tgt_key_padding_mask = (
                tgt_key_padding_mask.expand(2, 3),
            ) * 2
        samples.append(
            ModuleInput(
                constructor_input=FunctionInput(
...
def module_inputs_torch_nn_Transformer(
    ...
    for mask, key_padding_mask, norm_first, bias, batch_first in itertools.product(
        attn_masks, key_padding_masks, (True, False), (True, False), (True, False)
    ):
        # Using same mask for tgt and memory
...
_macos15_or_newer = (
    torch.backends.mps.is_available() and torch.backends.mps.is_macos_or_newer(15, 0)
)
...
module_db: list[ModuleInfo] = [
    ...
    ModuleInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                skipIfMPS, "TestModule", dtypes=[torch.float16], device_type="mps"
            ),
        ),
    ),
    ModuleInfo(
    ...
    ModuleInfo(
        ...
        decorators=(
            ...
            DecorateInfo(unittest.expectedFailure, "TestModule", "test_memory_format"),
            DecorateInfo(
                toleranceOverride({torch.float16: tol(atol=3e-2, rtol=1e-3)}),
                "TestModule",
                "test_forward",
                dtypes=[torch.float16],
                device_type="cpu",
    ...
    ModuleInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                unittest.skip("Skipped!"), "TestModule", "test_non_contiguous_tensors"
            ),
        ),
    ),
    ModuleInfo(
    ...
    ModuleInfo(
        ...
        skips=(
            ...
            DecorateInfo(
                expectedFailureMPS, "TestModuleMPS", "test_non_contiguous_tensors"
            ),
