...
cute::Layout<cute::Shape<int,int,int>, {operation_name_str}_StrideNarrow>{{}}));
            ...
            values = {
                ...
                "cluster_shape_m": (
                    "cute::_" + str(operation.tile_description.cluster_shape[0])
                    if operation.tile_description.cluster_shape[0] > 0
                    else "int"
                ),
                "cluster_shape_n": (
                    "cute::_" + str(operation.tile_description.cluster_shape[1])
                    if operation.tile_description.cluster_shape[1] > 0
                    else "int"
                ),
                "cluster_shape_k": (
                    "cute::_" + str(operation.tile_description.cluster_shape[2])
