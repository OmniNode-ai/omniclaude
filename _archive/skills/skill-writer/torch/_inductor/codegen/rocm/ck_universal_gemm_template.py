    ...
    def render(  # type: ignore[override]
        ...
        **kwargs,
        ...
        Bias = (
            ...
            else self.input_nodes[4] if 5 == len(self.input_nodes) else None
        )
...
"""
        ...
        res = self._template_from_string(self.gemm_template).render(
            ...
            ds_size=(
                ...
                if op.c_elementwise_op in ("Bilinear", "ScaleAdd")
                    ...
                    else 3 if op.c_elementwise_op == "MultiplyMultiplyAdd" else 0
                )
            ),
            ds_names=", ".join(
                ["Bias"]
                if op.c_elementwise_op in ("Bilinear", "ScaleAdd")
                    ...
                    ["inv_scale_x", "inv_scale_w"]
                    if op.c_elementwise_op == "MultiplyMultiply"
                    else (
                        ["inv_scale_x", "inv_scale_w", "Bias"]
        ...
        if config.rocm.generate_test_runner:
            ...
            runner_code = self._template_from_string(
                ...
                scale_a_ck_dtype=(
                    op.ds_element_dtypes[0]
                    if has_scale and 2 == len(op.ds_element_dtypes)
                    else "BF16"
                ),
                scale_b_ck_dtype=(
                    op.ds_element_dtypes[1]
                    if has_scale and 2 == len(op.ds_element_dtypes)
                    else "BF16"
                ),
                ...
                bias_torch_dtype=(
                    DTYPE_TO_CPP[Bias.get_layout().dtype] if Bias is not None else ""
                ),
                scale_a_torch_dtype=(
                    DTYPE_TO_CPP[scale_x.get_layout().dtype]
                    if scale_x is not None
                    else ""
                ),
                scale_b_torch_dtype=(
                    DTYPE_TO_CPP[scale_w.get_layout().dtype]
                ...
                bias_layout=(
                    torch_layout_to_ck_layout(Bias.get_layout())
                    if Bias is not None
                    else ""
                ),
    ...
    def gen_ops(self) -> list[InductorROCmOp]:
        ...
        try:
            from ck4inductor.batched_universal_gemm.gen_instances import (
                gen_ops_library as gen_batched_gemm_ops_library,  # type: ignore[import]
            )
            from ck4inductor.universal_gemm.gen_instances import (
                gen_ops_library as gen_gemm_ops_library,  # type: ignore[import]
            )
            from ck4inductor.universal_gemm.gen_instances import (
    ...
    def size_args(self):
        ...
        Bias = (
            ...
            else self.input_nodes[4] if len(self.input_nodes) == 5 else None
        )
        Y = self.output_node
