...
class FxConverter:
    ...
    def __post_init__(self) -> None:
        ...
        self.buffer_to_node: dict[Optional[str], torch.fx.Node] = (
            {}
    ...
    def _lookup_args(self, args: tuple[Any, ...]) -> tuple[Any, ...]:
        ...
        return tuple(
            (
                ...
                else arg.inner_expr if isinstance(arg, SymbolicCallArg) else arg
            )
            for arg in args
        )
    ...
    def _generate_triton_call(self, line: WrapperLine) -> None:
        ...
        # pyrefly: ignore [missing-attribute]
        assert not any(
            kwarg in kernel_config.kwargs for kwarg in call_kwargs
