...
def _sample_entry() -> dict:
    """Return a realistic session registry entry dict."""
    return {
        "task_id": "OMN-1234",
        "status": "active",
...
class TestFormatResumeContext:
    ...
    def test_format_includes_task_id(self) -> None:
        ...
        context = client.format_resume_context(_sample_entry())
    ...
    def test_format_handles_empty_lists(self) -> None:
        ...
        client = SessionRegistryClient(db_url=None)
        context = client.format_resume_context(entry)
    ...
    def test_format_handles_string_last_activity(self) -> None:
        """last_activity may come as string from raw dict."""
        entry = _sample_entry()
        entry["last_activity"] = "2026-03-27T14:00:00Z"
        client = SessionRegistryClient(db_url=None)
        context = client.format_resume_context(entry)
        assert "2026-03-27T14:00:00Z" in context
...
class TestGetSessionD4:
    ...
    def test_returns_found_when_entry_exists(self) -> None:
        ...
        mock_cursor.description = [
            MagicMock(name="task_id"),
            MagicMock(name="status"),
            MagicMock(name="current_phase"),
            MagicMock(name="worktree_path"),
            MagicMock(name="files_touched"),
            MagicMock(name="depends_on"),
            MagicMock(name="session_ids"),
            MagicMock(name="correlation_ids"),
            MagicMock(name="decisions"),
            MagicMock(name="last_activity"),
            MagicMock(name="created_at"),
        ]
        # psycopg2 cursor description uses .name attribute
