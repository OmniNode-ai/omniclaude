...
"""Unit tests for the agent-routing stack — restored under OMN-18805.

Restored, not written. This file was added at ``agents/tests/`` by ``7f4d00c4c``
and deleted by ``9f9d3369d`` on 2025-10-29. The ``Agent Framework Tests`` CI job
kept naming it for the next seven months and reported success from an empty
synthetic report every run, which is the false green epic OMN-18775 exists to
remove.

Every module it exercises is still on ``dev`` under ``src/omniclaude/lib/core/``,
so the coverage was lost rather than moved, and restoring it is cheaper than
writing it again. Exactly two mechanical adaptations were needed against the
last commit that carried it: the import root moved from ``lib`` to
``omniclaude.lib.core``, and ``EnhancedTriggerMatcher`` is now
``TriggerMatcher``. No assertion was changed, relaxed or deleted — all 30 tests
and all 74 assertions pass against today's implementation.

Its two former peers in that job, ``test_quality_gates.py`` and
``test_performance_thresholds.py``, were deliberately NOT restored: they were 26
and 36 bare skip markers with zero assertions between them, so bringing
them back would replace one false green with another.
...
from omniclaude.lib.core.capability_index import CapabilityIndex
from omniclaude.lib.core.confidence_scorer import ConfidenceScore, ConfidenceScorer
