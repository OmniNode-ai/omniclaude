...
from agents.lib.embedding_search import (
    find_similar_errors,
    get_error_patterns_analysis,
    recommend_stfs_for_error,
)
...
async def test_embedding_search():
    try:
        ...
        similar_errors = await find_similar_errors(
            error_text="Task breakdown validation failed",
        ...
        for error in similar_errors:
            print(
                f"  - {error.error_type}: {error.message[:50]}... (similarity: {error.similarity_score:.3f})"
            )
        ...
        for rec in recommendations:
            print(
                f"  - {rec['stf_name']}: confidence={rec['confidence']:.3f}, success_rate={rec['success_rate']:.3f}"
            )
        ...
        for pattern in patterns[:3]:
            print(
                f"  - {pattern['error_type']}: {pattern['message'][:50]}... (freq: {pattern['frequency']})"
            )
