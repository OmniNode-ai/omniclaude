...
class TestPolicyLoading:
    ...
    def test_policy_covers_full_streak_range(self, policy: ModelEscalationPolicy) -> None:
    ...
    def test_policy_fsm_transitions_complete(self, policy: ModelEscalationPolicy) -> None:
...
class TestReducer:
    ...
    def test_success_emits_no_intents(
        ...
        state, intents = reduce(state, _make_event(result="pass", phase="complete"), policy)
...
class TestIndependentLoops:
    ...
    def test_success_one_doesnt_reset_other(
        ...
        for _ in range(2):
            state, _ = reduce(state, _make_event(loop="closeout"), policy)
        state, _ = reduce(
            state, _make_event(loop="buildloop", result="pass", phase="complete"), policy
        )
        assert state.loops["closeout"].escalation_level == 2
...
class TestActionRecording:
    ...
    def test_actions_prepended(
        ...
        state, _ = reduce(empty_state, _make_action_event(action="restarted", detail="first"), policy)
        state, _ = reduce(state, _make_action_event(action="investigated", detail="second"), policy)
...
class TestCheckEscalation:
    ...
    def test_includes_last_error(self, empty_state: ModelWatchdogState, policy: ModelEscalationPolicy) -> None:
        state = empty_state
        for _ in range(2):
            state, _ = reduce(state, _make_event(error="PostgreSQL unreachable"), policy)
        result = check_escalation(state, "closeout")
        assert result.last_error == "PostgreSQL unreachable"

    def test_includes_fsm_state(self, empty_state: ModelWatchdogState, policy: ModelEscalationPolicy) -> None:
        state, _ = reduce(empty_state, _make_event(), policy)
        result = check_escalation(state, "closeout")
...
class TestShellScripts:
    ...
    def _run(self, script: str, args: list[str], omni_home: Path) -> subprocess.CompletedProcess[str]:
    ...
    def test_write_failure(self, clean_state: Path) -> None:
        r = self._run("watchdog-state-write.sh", ["closeout", "fail", "B1", "pg down"], clean_state)
    ...
    def test_write_success_resets(self, clean_state: Path) -> None:
        for _ in range(3):
            self._run("watchdog-state-write.sh", ["closeout", "fail", "B1", "err"], clean_state)
        r = self._run("watchdog-state-write.sh", ["closeout", "pass", "complete", ""], clean_state)
        assert "escalation=0" in r.stdout

    def test_write_invalid_loop(self, clean_state: Path) -> None:
        r = self._run("watchdog-state-write.sh", ["invalid", "fail", "B1", "err"], clean_state)
        assert r.returncode != 0

    def test_write_invalid_result(self, clean_state: Path) -> None:
        r = self._run("watchdog-state-write.sh", ["closeout", "invalid", "B1", "err"], clean_state)
        assert r.returncode != 0
    ...
    def test_check_json_includes_fsm_state(self, clean_state: Path) -> None:
        self._run("watchdog-state-write.sh", ["closeout", "fail", "B1", "pg down"], clean_state)
        self._run("watchdog-state-write.sh", ["closeout", "fail", "B1", "pg down"], clean_state)
        r = self._run("watchdog-check.sh", ["closeout"], clean_state)
    ...
    def test_record_action(self, clean_state: Path) -> None:
        self._run("watchdog-state-write.sh", ["closeout", "fail", "B1", "err"], clean_state)
        r = self._run("watchdog-record-action.sh", ["closeout", "restarted", "auto"], clean_state)
        assert r.returncode == 0
    ...
    def test_independent_loops(self, clean_state: Path) -> None:
        self._run("watchdog-state-write.sh", ["closeout", "fail", "B1", "err"], clean_state)
        self._run("watchdog-state-write.sh", ["closeout", "fail", "B1", "err"], clean_state)
        self._run("watchdog-state-write.sh", ["buildloop", "fail", "exec", "err"], clean_state)
