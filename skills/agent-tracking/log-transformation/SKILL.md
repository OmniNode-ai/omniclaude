...
description: Log agent transformation events to PostgreSQL for observability and learning. Records when agents transform from one identity to another, including success status and duration.
---

# Log Transformation

This skill logs agent transformation events to the PostgreSQL database for tracking polymorphic agent behavior, observability, and performance analysis.
...
Logs to: `agent_transformation_events`
...
Use the Bash tool to execute the logging script with your actual transformation parameters:
...
This skill is part of the agent observability system. Logged transformations are:
...
- Consider using Kafka version for async logging (coming soon)
