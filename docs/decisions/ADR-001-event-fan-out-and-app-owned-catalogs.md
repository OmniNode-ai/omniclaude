...
| Legacy in-process publisher | Replaced infra `EventRegistry` with local registry imports, added `_inject_metadata()`, fan-out loop in `_handle_emit()` |
| `plugins/onex/hooks/lib/hook_event_adapter.py` | `TopicBase.ROUTING_DECISIONS` → `TopicBase.ROUTING_DECISION` |
| Legacy publisher tests | Removed all `patch.object(publisher._registry, ...)` mocks |
...
# Tests pass
.venv/bin/pytest tests/hooks/test_event_registry.py tests/hooks/test_topics.py -v
```
...
- **Publisher**: legacy in-process publisher — fan-out loop in `_handle_emit()`
