...
if [ -f "/Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh" ]; then
...
CORR_LINE=$(grep -n "Correlation ID (generated before agent detection)" /Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh | cut -d: -f1)
AGENT_LINE=$(grep -n "Agent detection via Router Service" /Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh | cut -d: -f1)
...
if grep -q "curl -s -X POST http://localhost:8055/route" /Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh && \
   grep -q "\-\-connect-timeout 2" /Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh && \
   grep -q "\-\-max-time 5" /Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh; then
    echo -e "${GREEN}✓ PASS${NC}"
    PASSED=$((PASSED + 1))
else
...
if grep -q "Router service unavailable - using fallback" /Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh && \
   grep -q "SERVICE_USED=\"false\"" /Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh; then
    echo -e "${GREEN}✓ PASS${NC}"
    PASSED=$((PASSED + 1))
else
...
if grep -q "jq -r '.selected_agent // \"NO_AGENT_DETECTED\"'" /Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh && \
   grep -q "jq -r '.confidence // \"0.5\"'" /Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh; then
    echo -e "${GREEN}✓ PASS${NC}"
    PASSED=$((PASSED + 1))
else
...
if grep -q "service=\${SERVICE_USED}" /Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh; then
    echo -e "${GREEN}✓ PASS${NC}"
    PASSED=$((PASSED + 1))
else
...
CORR_COUNT=$(grep -c "Correlation ID" /Volumes/PRO-G40/Code/omniclaude/claude_hooks/user-prompt-submit.sh | grep -v "Correlation ID:" || echo "1")
...
if [ -f "/Volumes/PRO-G40/Code/omniclaude/claude_hooks/test_router_service_integration.sh" ] && \
   [ -x "/Volumes/PRO-G40/Code/omniclaude/claude_hooks/test_router_service_integration.sh" ]; then
    echo -e "${GREEN}✓ PASS${NC}"
    PASSED=$((PASSED + 1))
else
...
if [ -f "/Volumes/PRO-G40/Code/omniclaude/docs/HOOK_ROUTER_SERVICE_INTEGRATION.md" ]; then
    echo -e "${GREEN}✓ PASS${NC}"
    PASSED=$((PASSED + 1))
else
