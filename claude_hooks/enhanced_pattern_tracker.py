#!/usr/bin/env python3
"""
Enhanced Performance Pattern Tracker - Optimized pattern tracking with performance monitoring.
...
class EnhancedPatternTrackerConfig:
...
class PerformanceMonitor:
    ...
    def record_operation(self, operation: str, success: bool, duration_ms: float, api_response_time_ms: Optional[float] = None):
...
class BatchProcessor:
    ...
    async def add_task(self, task_type: str, **kwargs):
        ...
        try:
            ...
            print(f"Warning: Batch processor queue full, dropping task")
    ...
    async def _batch_track_creation(self, tasks: List[Dict]):
        ...
        for task in tasks:
            coroutines.append(
                self.tracker.track_pattern_creation(**task)
            )
...
class EnhancedPatternTracker:
    """Enhanced pattern tracker with performance optimizations."""
    ...
    def __init__(self, config: Optional[EnhancedPatternTrackerConfig] = None):
        self.config = config or EnhancedPatternTrackerConfig()
    ...
    def _setup_logging(self):
        ...
        log_file = self.config.log_file if hasattr(self.config, 'log_file') else \
                   Path.home() / ".claude" / "hooks" / "logs" / "enhanced-pattern-tracker.log"
    ...
    async def track_pattern_creation(
        ...
        if use_batch and self.config.batch_config.enabled:
            await self.batch_processor.add_task("track_pattern_creation",
    ...
    async def track_pattern_creation_batch(
        self,
        patterns: List[Tuple[str, Dict[str, Any], Optional[Dict[str, Any]]]]
    ) -> List[str]:
...
_enhanced_tracker_instance: Optional[EnhancedPatternTracker] = None
_tracker_lock = threading.Lock()


def get_enhanced_tracker() -> EnhancedPatternTracker:
    """Get global enhanced pattern tracker instance."""
    global _enhanced_tracker_instance
    if _enhanced_tracker_instance is None:
        with _tracker_lock:
            if _enhanced_tracker_instance is None:
                _enhanced_tracker_instance = EnhancedPatternTracker()
    return _enhanced_tracker_instance


# Export public API
__all__ = [
    'EnhancedPatternTracker',
    'EnhancedPatternTrackerConfig',
    'PerformanceMetrics',
    'ProcessingMode',
    'PerformanceMonitor',
    'BatchProcessor',
