...
```
Bash(./ci-quick-review 33)
Bash(./fetch-ci-data 33)
```
...
Task(
  ...
  prompt="Analyze CI failures for PR #33. Use the ci-failures skill tools:
    1. Run: ./ci-quick-review 33
    2. For deep investigation, use: ./get-ci-job-details <job_id>
...
./ci-quick-review my-feature-branch

# Filter to specific workflow
./ci-quick-review --workflow ci-cd 33

# JSON output for scripting
./ci-quick-review --json 33 > ./tmp/ci-failures.json
```
...
```bash
# By job ID
./get-ci-job-details 56174634733
...
./get-ci-job-details 56174634733 33

# Get job ID from ci-quick-review output, then investigate
./ci-quick-review 33
./get-ci-job-details <job_id_from_output>
```
...
./fetch-ci-data my-feature-branch

# Fetch for current branch
./fetch-ci-data

# Filter to specific workflow
./fetch-ci-data --workflow enhanced-ci 33
...
./ci-quick-review --json 33 | jq '.failures[] | select(.severity == "critical")'

# Get failure summary for specific workflow
./fetch-ci-data --workflow ci-cd 33 | jq '.summary'

# Compare failures across multiple PRs
for pr in 32 33 34; do
  echo "PR $pr:"
  ./fetch-ci-data $pr | jq -r '.summary | "\(.total) failures (\(.critical) critical, \(.major) major)"'
done
```
    ...
    steps:
      ...
      - name: Analyze CI Failures
        run: |
          ...
          ./ci-quick-review ${{ github.event.pull_request.number }}

          # Or post JSON summary to PR
          if [ "${{ github.event_name }}" == "pull_request" ]; then
            SUMMARY=$(./ci-quick-review ${{ github.event.pull_request.number }})
...
**Executables**:
- `./ci-quick-review` - One-command quick review with concise summary (RECOMMENDED - Tier 1)
- `./get-ci-job-details` - Deep-dive investigation into specific job failures (Tier 2)
- `./fetch-ci-data` - Fetch CI failure data with severity classification (Advanced)
...
**Debugging & Analysis**:
- `../pr-review/` - PR review and feedback analysis
...
./ci-quick-review --workflow enhanced-ci 33
```
...
./fetch-ci-data 33 > ./tmp/ci-data.json
...
./get-ci-job-details 56174634821
...
- System Status Skills: `../system-status/SKILL.md`
- Event Alignment Plan: `/docs/events/EVENT_ALIGNMENT_PLAN.md`
