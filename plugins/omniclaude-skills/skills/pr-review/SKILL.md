...
```
Bash(./collate-issues 30)
Bash(./pr-quick-review 22)
```
...
./pr-quick-review 22 --save ./my-review.md

# JSON output for scripting
./pr-quick-review 22 --json > pr22.json

# CI/CD mode (fails if issues found)
./pr-quick-review 22 --strict
```
...
```bash
# Save to specific file
./review-pr 22 --output-file ./tmp/pr22-review.md

# JSON output for programmatic processing
./review-pr 22 --json > pr22.json
```
...
- name: PR Review
  run: |
    ./review-pr ${{ github.event.pull_request.number }} --strict
...
**Claude Code Access**: `./`
**Executables**:
- `./pr-quick-review` - One-command quick review (RECOMMENDED)
- `./fetch-pr-data` - Fetch all PR data from 4 GitHub endpoints
- `./analyze-pr-comments` - Pre-process raw data into categorized analysis (NEW)
- `./review-pr` - Comprehensive review with priority organization
- `./pr-review-production` - Production-grade wrapper (NEW)
