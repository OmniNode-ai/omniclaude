...
adversarial_pipeline, multi_agent, integration_sweep, ticketing_epic_org,
...
**2026-07-01 (<TICKET>, M2c disposition of the 5 no-node facades from <TICKET>):**
...
- `onex:checkpoint` — verdict corrected from FACADE to **WORKS (misclassified)**.
  ...
  repoint the registry / retire the dead omniclaude Polly-shell: **<TICKET>**.
- `onex:dispatch_watchdog` — **FACADE**, de-listed from event-bus dispatch (no
  ...
  `node_worker_stall_recovery`). Tracked: **<TICKET>**.
- `onex:epic_team` — **FACADE**, de-listed from event-bus dispatch by design
  ...
  LLM session, not a node). Tracked: **<TICKET>**.
- `onex:executing_plans` — **FACADE**, de-listed from event-bus dispatch by
  design (session-native plan review + routing decision). Tracked: **<TICKET>**.
- `onex:wave_scheduler` — **FACADE**, de-listed from event-bus dispatch by
  ...
  COMPUTE-node candidate). Tracked: **<TICKET>**.
