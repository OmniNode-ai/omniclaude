...
Run ONE command. It prints exactly one typed `ModelSkillResult[CoverageSweepResult]`
...
```bash
uv run onex skill coverage_sweep [--repos <a,b>] [--target-pct <n>] [--dry-run]
```
...
- **Result**: `result` — the full `CoverageSweepResult`; surface its fields directly.
