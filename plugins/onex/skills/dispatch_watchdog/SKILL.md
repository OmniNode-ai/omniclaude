...
**Prefer `onex:agent_healthcheck` instead** — it is real node-backed: detection and recovery logic lives in `node_worker_stall_recovery` (omnimarket, <TICKET>). This skill's stall-detection semantics substantially overlap with `agent_healthcheck`'s; consolidation is tracked in <TICKET>.
