...
After receiving `result.validated_prompt_template`, prepend the Operating Rules block
verbatim (worker_template_version: v2). This is non-negotiable — every worker prompt MUST
begin with these rules regardless of role or spec contents.

The block is defined once, canonically, and referenced here rather than restated:

@_lib/dispatch-laws/helpers.md

Do NOT restate the rules in this file. A second hand-maintained copy drifts -- that is
exactly the failure this reference replaces. Amend the rules in the pack, never here.
