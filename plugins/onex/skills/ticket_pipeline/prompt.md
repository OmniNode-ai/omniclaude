...
**Populate dod_evidence[] from implementation artifacts.**

This phase enriches the skeleton contract generated by `generate_contract` with actual
evidence items discovered from the implementation. It runs after implement and before
local_review. Failure is non-fatal -- the pipeline continues with an unenriched contract.

1. Auto-detect onex_change_control repo path (same pattern as generate_contract):
   ```bash
   if [ -z "$ONEX_CC_REPO_PATH" ]; then
     TICKET_ID_SHORT=$(git branch --show-current | grep -oE 'OMN-[0-9]+')
     WORKTREE_BASE="${ONEX_WORKTREE_ROOT:-/Volumes/PRO-G40/Code/omni_worktrees}"  # local-path-ok
...
2. Check if contract exists: `ls $ONEX_CC_REPO_PATH/contracts/{ticket_id}.yaml 2>/dev/null`
   - If not exists: skip this phase (no contract to enrich)
   - If exists: continue

3. Discover test files:
   ```bash
   TEST_FILES=$(git diff --name-only origin/main...HEAD -- 'tests/' | grep '\.py$')
   ```

4. Build test command:
   ```bash
   if [ -n "$TEST_FILES" ]; then
     TEST_CMD="uv run pytest $TEST_FILES -v"
   else
     TEST_CMD=""
   fi
   ```
...
5. Enrich contract:
   ...
   with open(f"{onex_cc_path}/contracts/{ticket_id}.yaml") as f:
   ...
   with open(f"{onex_cc_path}/contracts/{ticket_id}.yaml", "w") as f:
       f.write(enriched)
   ```

6. Commit:
   ```bash
   cd $ONEX_CC_REPO_PATH && git add contracts/{ticket_id}.yaml && git commit -m "contract: enrich {ticket_id} with dod_evidence [auto-generated]"
   ```

**On failure:** Log warning "Contract enrichment failed: {error}". Continue pipeline.

**State artifacts:**
- `phases.enrich_contract.started_at`
- `phases.enrich_contract.completed_at`
- `phases.enrich_contract.artifacts` (evidence_count, test_files_found, skipped)

**Exit conditions:**
- **Completed:** Contract enriched with evidence items (AUTO-ADVANCE)
- **Skipped:** No contract exists to enrich (AUTO-ADVANCE)
- **Failed:** Non-fatal warning, pipeline continues (AUTO-ADVANCE)
