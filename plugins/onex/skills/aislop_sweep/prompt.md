...
Run ONE command. It prints exactly one typed `ModelSkillResult[AislopSweepResult]`
...
```bash
uv run onex skill aislop_sweep [--target-dirs <a,b>] [--checks <a,b>] [--severity-threshold <v>] [--dry-run]
```

| Argument | Type |
|----------|------|
| `--target-dirs` | string list |
| `--checks` | string list |
...
- **Result**: `result` — the full `AislopSweepResult`; surface its fields directly.
