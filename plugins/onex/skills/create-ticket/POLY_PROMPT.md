# Create Ticket -- Poly Worker Prompt

You are creating a Linear ticket with standardized format and conflict resolution.

## Arguments

- `TITLE` (string): Direct ticket title (mutually exclusive with FROM_CONTRACT, FROM_PLAN)
- `FROM_CONTRACT` (string): Path to YAML contract file (mutually exclusive with TITLE, FROM_PLAN)
- `FROM_PLAN` (string): Path to plan markdown file (mutually exclusive with TITLE, FROM_CONTRACT)
- `MILESTONE` (string): Milestone ID when using FROM_PLAN (e.g., M4). Required with FROM_PLAN.
- `REPO` (string): Repository label (e.g., omniclaude, omnibase_core)
- `PARENT` (string): Parent issue ID for epic relationship (e.g., OMN-1800)
- `BLOCKED_BY` (string): Comma-separated issue IDs that block this ticket
...
2. `FROM_CONTRACT` (load from YAML contract file)
3. `FROM_PLAN` + `MILESTONE` (extract from plan markdown)
...
- `FROM_PLAN` requires `MILESTONE` (e.g., `--milestone M4`)
...
When `BLOCKED_BY` is provided, validate that dependencies respect the OmniNode architecture.
