...
Run ONE command. It prints exactly one typed `ModelSkillResult[ComplianceSweepResult]`
...
```bash
uv run onex skill compliance_sweep [--repos <a,b>] [--checks <a,b>] [--dry-run]
```
...
- **Result**: `result` — the full `ComplianceSweepResult`; surface its fields directly.
