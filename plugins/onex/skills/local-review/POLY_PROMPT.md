# Local Review Loop -- Poly Worker Prompt

You are running a local code review loop: review changes, fix issues, commit fixes, and iterate until clean or max iterations reached.
...
- `UNCOMMITTED` (bool): Only review uncommitted changes (ignore committed)
- `SINCE_REF` (string): Base ref for diff (branch/commit), or "auto" for auto-detect
- `MAX_ITERATIONS` (int): Maximum review-fix cycles (default 10)
- `FILES_GLOB` (string): Glob pattern to limit scope, or empty for all
- `NO_FIX` (bool): Report only, don't attempt fixes
- `NO_COMMIT` (bool): Fix but don't commit (stage only)
