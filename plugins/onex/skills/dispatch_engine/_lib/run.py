...
"""dispatch_engine skill — dispatch through runtime ingress.

Invoked when the user runs /onex:dispatch_engine. Builds a ModelSkillRequest
payload and dispatches to the runtime via transport priority:
  ...
  3. Kafka (contract-driven, topic onex.cmd.omnimarket.dispatch_engine.v1)
...
# ---------------------------------------------------------------------------
# sys.path setup — mirror the merge_sweep skill pattern exactly
# ---------------------------------------------------------------------------
_LIB_DIR = Path(__file__).parent  # dispatch_engine/_lib/
_SKILL_DIR = _LIB_DIR.parent  # dispatch_engine/
...
_CONTRACT_RELATIVE_PATH = "omnimarket/src/omnimarket/nodes/node_skill_dispatch_engine_orchestrator/contract.yaml"
...
def _resolve_command_topic() -> str:
    ...
    try:
        ...
        topic = (data or {}).get("event_bus", {}).get("subscribe", {}).get("topic", "")
...
def _dispatch_via_kafka(
    ...
    envelope = {
        ...
        "source": "omniclaude.dispatch-engine-skill",
        "correlation_id": correlation_id_str,
        "payload": payload,
    }
...
def dispatch_dispatch_engine(
    *,
    ...
    """Dispatch node_skill_dispatch_engine_orchestrator via runtime ingress.
    ...
    if ssh_host and ssh_socket_path:
        ssh_payload = {
            "command_name": _DISPATCH_ENGINE_COMMAND_NAME,
            "payload": payload,
            "correlation_id": correlation_id_str,
                ...
                if is_structured:
                    return {
                        ...
                        "command_name": getattr(
                            response, "command_name", _DISPATCH_ENGINE_COMMAND_NAME
                        ),
...
def main() -> None:
    """CLI entry point for /onex:dispatch_engine."""
    import argparse  # noqa: PLC0415

    parser = argparse.ArgumentParser(
        description="dispatch_engine skill — dispatch through runtime SSH socket, HTTP, or Kafka"
    )
    ...
    skill_path = str(_SKILL_DIR / "SKILL.md")

    result = dispatch_dispatch_engine(
        skill_path=skill_path,
    ...
    if result.get("success"):
        print(
            f"\ndispatch_engine dispatched ({result.get('path')}) - "
        ...
        print(
            f"\ndispatch_engine dispatch failed: {result.get('error')}", file=sys.stderr
        )
        sys.exit(1)
